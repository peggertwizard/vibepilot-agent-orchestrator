...
**Nothing waits on a model to notice.** A step that finishes starts the next one; a step that
dies is found by the heartbeat and restarted. Prompts describe intent — mechanisms carry it.
...
- **One "Needs you" list** — questions, sign-off gates, ticket proposals, breakdowns, merges,
  stalled teammates and hires all in one popover, reachable from every tab. Most resolve in
  place; the rest take you to where they do. The badge counts exactly what the list holds
...
- **Auto-start** (off / one at a time / always), **auto-merge** (off / on green checks /
  always) and **auto-archive** (finished tickets leave the board after N days), per project
- **Self-healing**: a step that claims to be running with no process behind it is restarted
  once, resuming its session; a second failure asks you instead of retrying
...
That runs `vitest run` — the whole suite once, no watcher. **361 tests across 39 files**,
about 85 seconds, because a few of them are real.

Most are ordinary unit tests over the engine: board placement, routing and gates, merge
decisions, branch grouping, stuck-step detection and healing, auto-archive, the machine-owned
file classification, migrations, memory. Three
