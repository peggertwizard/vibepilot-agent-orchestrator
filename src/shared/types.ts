 ...
 *
 * Repeated kinds are named by *who* does them, because otherwise they are indistinguishable.
 * A real route — a writer producing the copy, then a developer building the fields and
 * migration behind it, then a review — is three genuinely different jobs and rendered as
 * **"Build → Build → Review"**, which reads as a duplicated step rather than a sequence. The
 * Pilot described the same route in prose as "Writer → Senior Dev → Review" and was clearer
 * than the app.
 *
 * Only when a kind actually repeats. "Research → Build → Review" is already unambiguous and
 * gains nothing from three names.
 */
export function routeSummary(
  steps: Pick<RouteStep, 'kind' | 'assigneeAgentId'>[],
  /** Supplied by callers that have the roster to hand. Without it, nothing changes. */
  nameOf?: (agentId: string) => string | null | undefined,
): string {
  const seen = new Map<string, number>()
  for (const s of steps) seen.set(s.kind, (seen.get(s.kind) ?? 0) + 1)

  return (
    steps
      .map((s) => {
        const label = STEP_LABEL[s.kind]
        if ((seen.get(s.kind) ?? 0) < 2) return label
        const who = s.assigneeAgentId && nameOf ? nameOf(s.assigneeAgentId) : null
        return who || label
      })
      .join(' → ') || '—'
  )
...
export const CODEX_LIMITATIONS = [
  ...
  'reports what it sent but not the size of its context window, so there is a number and no bar',
