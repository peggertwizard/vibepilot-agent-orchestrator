...
/**
 * What an agent's usage is worth against your rate limit.
 *
 * Deliberately NOT the raw sum. One vibePilot turn is one prompt over many API
 * round-trips, and every round-trip re-reads the whole conversation as a cache
 * read — so cache_read counts the same content once per round-trip. Summing the
 * four fields raw reported 9.27M for a run whose distinct content was ~225k.
 */
...
/** The raw four, for a tooltip that explains where the weighted figure came from. */
export function tokenBreakdown(
  a: Pick<Agent, 'tokensIn' | 'tokensOut' | 'tokensCacheRead' | 'tokensCacheWrite'>
): string {
  const n = (v: number): string => v.toLocaleString('en-US')
  return (
    `${n(a.tokensIn)} in · ${n(a.tokensOut)} out · ${n(a.tokensCacheWrite)} cache written · ` +
    `${n(a.tokensCacheRead)} cache read\n\n` +
    'Cache reads are the conversation re-sent on every API call, not distinct content, ' +
    'so they count for a tenth. Output counts for five.'
  )
}
