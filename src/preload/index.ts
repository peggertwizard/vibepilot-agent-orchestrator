...
const api = {
  ...
  agents: {
    ...
    /**
     * Throw away what a teammate is carrying and start it clean.
     *
     * The escape hatch for a teammate that is wedged for a reason nobody wants to diagnose.
     * Costs its conversation; never its branch, worktree or commits.
     */
    reset: (agentId: string) =>
      invoke<{ ok: boolean; started?: boolean; reason?: string }>('agents:reset', { agentId }),
    create: (input: {
      projectId: string
      name: string
      role: string
  ...
  git: {
    ...
    /** Squash-merge whatever branch the folder is on into the base. No ticket needed. */
    mergeCurrent: (projectId: string) =>
      invoke<{ ok: boolean; sha?: string; reason?: string }>('git:mergeCurrent', { projectId }),
    /** Delete ticket branches whose work is already on the base. Never automatic. */
    deleteLandedBranches: (projectId: string) =>
      invoke<{ ok: boolean; deleted?: string[]; kept?: string[]; reason?: string }>(
        'git:deleteLandedBranches',
        { projectId },
      ),
    /** Put the project folder back on the base branch. Refuses rather than forcing. */
    checkoutBase: (projectId: string) =>
      invoke<{ ok: boolean; from?: string; reason?: string }>('git:checkoutBase', { projectId }),
  ...
  settings: {
    ...
    /** Where the Codex binary is. Same contract as the Claude pair above. */
    codexBinary: () => invoke<string | null>('settings:codexBinary'),
    setCodexBinary: (path: string) => invoke<boolean>('settings:setCodexBinary', { path }),
  },
  codex: {
    /**
     * The models the installed Codex reports, with the efforts each one accepts.
     *
     * Empty means no Codex, or one too old to be asked — the caller offers a plain text field
     * rather than pretending to know a list.
     */
    models: () => invoke<CodexModel[]>('codex:models'),
  },
  /**
