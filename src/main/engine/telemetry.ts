import { bus } from '../bus'
import { clearAgentSession, forgetSession, recordAgentUsage, setResolvedModel } from '../db/repos/agents'
...
export function installTelemetry(): void {
  ...
  bus.onAgent((e) => {
    switch (e.type) {
      ...
      case 'agent:error': {
        /*
         * The CLI names the session it could not find, so forget precisely that one.
         *
         * 0.7.3 cleared the agent row instead, which is not where resuming reads from — the
         * next restart pulled the same dead id out of `agent_runs` and failed the same way.
         * Matching the id out of the message keeps this surgical: other tickets' sessions,
         * which are fine, are untouched.
         */
        const dead = /No conversation found with session ID:?\s*([0-9a-f-]{8,})/i.exec(e.message)
        if (dead?.[1]) forgetSession(dead[1])
        else if (/No conversation found/i.test(e.message)) clearAgentSession(e.agentId)
        break
      }
