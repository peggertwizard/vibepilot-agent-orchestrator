...
import { place, sweepDoneTickets } from './board'
import { clearHealAttempts, healStuckSteps } from './heal'
...
export function startHeartbeat(): void {
  ...
  bus.onAgent((e) => {
    ...
    /*
     * A run that finished earns the next stall a fresh automatic attempt. Without this the
     * one-heal-per-app-run rule would spend an agent's only attempt on a stall in the morning
     * and leave it on its own for the rest of the day; with it, heal → immediate crash still
     * escalates, because no completion happened in between.
     */
    if (e.type === 'agent:done') clearHealAttempts(e.agentId)
  })
