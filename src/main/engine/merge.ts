...
import { currentBranch } from '../git/branches'
...
export async function mergeTicket(
  ...
  /*
   * Nothing left on the branch. Not a failure — the work landed some other way, or never
   * started. Merging it produces an empty commit and a ticket that claims to have done
   * something. `sweepEmptyReady` settles these properly.
   *
   * `hasLanded`, not `commitsAhead`, and that is the fix for the ghost merge card. A squash
   * merge copies the changes across as one new commit rather than carrying the originals, so
   * the branch keeps its own commits and the count keeps reporting them long after every line
   * is on the base. Three commits ahead, nothing missing. This guard never fired, the ticket
   * stayed "ready to merge" for ever, and pressing the button re-ran a merge that could only
   * fail.
   */
  if (await hasLanded(project.path, project.defaultBaseBranch, ticket.branch)) {
    await sweepEmptyReady(project.id)
    return {
      ok: false,
      kind: 'empty',
      reason: `\`${ticket.branch}\` has nothing left to merge — its work is already on ${project.defaultBaseBranch}.`,
      conflicts: [],
    }
  }
