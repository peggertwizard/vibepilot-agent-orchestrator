...
class RoutingService {
  ...
  private fire(projectId: string): void {
    ...
    /*
     * `needsPlanning` finally does something.
     *
     * It has been written by the create form and by the Pilot's own draft tool since v1,
     * stored on the ticket, and read by exactly one badge. A user ticking "plan first — I sign
     * off before anything is built" was recorded and then ignored, which is worse than not
     * offering the box. Said here, in the one prompt that decides the route.
     */
    const list = pending
      .map(
        (t) =>
          `- #${t.number} ${t.title}` +
          (t.needsPlanning
            ? ` — the user asked to plan this one first and sign off before it is built: ` +
              `route it plan then build, with needs_signoff on the build step`
            : ''),
      )
      .join('\n')
    pilot.notify(
   ...
   *
   * And then **start** the next step, rather than hoping the telling was enough.
   *
   * This is the fix for *"the ticket does not continue after the plan was created"*.
   * `completeActiveStep` is pure database: it marks the next step `active` and returns. The
   * only thing that ever put a process behind that step was the Pilot noticing this notice and
   * choosing to call `assign_teammate` — a hand-off through a language model, for a decision
   * that has no judgement in it. When the Pilot was busy, dead, rate-limited or simply chose
   * to answer the user instead, the step sat `active` with nobody on it and no event left to
   * wake anything.
   *
   * `startActiveStep` is the same launcher gate approval uses, so the queue, the concurrency
   * cap and the pause toggle all still apply — and a step with no assignee still returns null,
   * which remains the Pilot's to resolve.
   */
  announceStep(projectId: string, ticketNumber: number, ticketId: string): void {
    const route = acceptedRoute(ticketId)
    if (!route) return
    const started = startActiveStep(route)
    pilot.notify(
      projectId,
      `#${ticketNumber} moved on. ${describeNext(route)}` +
        (started ? ` ${started} has already been started on it — do not assign anyone.` : ''),
    )
  }
}
