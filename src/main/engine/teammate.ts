...
export async function launchTeammate(input: SpawnInput): Promise<void> {
  ...
  if (!project || !ticket) {
    setAgentStatus(input.agentId, 'error', 'Its ticket vanished before it could start')
  ...
  const spec: LaunchSpec = {
    ...
    effort: me?.effort ?? effortDefaultFor(input.role),
    agentId: input.agentId,
    projectId: input.projectId,
    ticketId: ticket.id,
  ...
  enqueueWrite(
    `INSERT INTO agent_runs (id, agent_id, project_id, ticket_id, provider, session_id, cwd, started_at)
     VALUES (?,?,?,?,?,?,?,?)`,
    runId,
    input.agentId,
    input.projectId,
