...
import { conversationExists } from '../providers/claude/session'
...
/**
 * The session id this run's conversation will actually carry.
 *
 * `argv.ts` passes `--session-id <minted>` on a cold start and `--resume <carried>` on a resume,
 * and **a resume never creates the minted id** — it keeps the conversation it resumed. Recording
 * the minted id either way wrote down a session no conversation would ever have, and
 * `decideCarry` reads the newest recorded id back out of `agent_runs` to decide what to resume
 * next time. One interruption then poisoned an agent permanently: resume a fabricated id, fail
 * with "No conversation found", record another fabricated id, repeat.
 *
 * Named and exported so the rule is asserted rather than reviewed.
 */
export function liveSessionId(carried: string | null, minted: string): string {
  return carried ?? minted
}
...
export async function launchTeammate(input: SpawnInput): Promise<void> {
   ...
   */
  const wanted = input.resumeSessionId
  ...
  /*
   * Only resume a conversation that is actually there.
   *
   * Everything above decides what it would *like* to resume. This is the one check that a
   * resume can survive being wrong about, and it was missing: whatever id had been recorded
   * went straight to `--resume`, and if no such conversation existed the launch died with
   * "No conversation found with session ID: …". The failed run then recorded another id, the
   * next restart resumed that, and the teammate was wedged permanently — four different dead
   * ids stacked up on one agent, each one manufactured by the attempt before it. The only
   * cure anyone found was deleting the teammate and adding it back, which worked because it
   * discarded the run history rather than because anything was repaired.
   *
   * The conversations are files. Ask the disk. A missing one costs the context and nothing
   * else: the branch, the commits and the brief are all still there to read.
   */
  const carry =
    wanted.sessionId && !conversationExists(worktree.path, wanted.sessionId)
      ? {
          sessionId: null,
          why: 'Starting fresh — the saved conversation is gone, but its branch is not.',
        }
      : wanted
  ...
  /*
   * The id the conversation will actually have — which is not always the one just minted.
   *
   * `argv.ts` passes `--session-id <minted>` on a cold start and `--resume <carried>` on a
   * resume, and **a resume never creates the minted id**: it keeps the conversation it resumed.
   * Recording the minted id either way wrote down a session that no conversation would ever
   * have, and `decideCarry` reads the newest recorded id back out of `agent_runs` to decide
   * what to resume next time.
   *
   * That is a loop that feeds itself. One interruption — "Interrupted when vibePilot closed" —
   * produced one resumed run, which recorded a fabricated id; the next restart resumed *that*,
   * got `No conversation found with session ID: …`, and its own failed run recorded another.
   * The message log alternated "Restarted: resuming the interrupted session" and "stopped"
   * indefinitely, and no retry policy could have helped, because every attempt was doomed
   * before it spawned.
   *
   * The condition is exactly the one `argv.ts` branches on, so it is the one recorded here.
   */
  const live = liveSessionId(carry.sessionId, sessionId)
  ...
  setAgentSession(input.agentId, live, worktree.path)
  enqueueWrite(
