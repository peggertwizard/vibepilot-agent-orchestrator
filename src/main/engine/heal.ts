...
/**
 * Make the roster tell the truth about itself when the app opens.
 *
 * Opening vibePilot showed a wall of alarming state that meant nothing. A teammate left in
 * `error` kept its last message for ever — *"No conversation found with session ID: 21285116…"*
 * from a session that had stopped mattering hours earlier — and one whose ticket had since been
 * cancelled still read "Interrupted when vibePilot closed", offering a Restart for work nobody
 * wanted. Neither was a live problem. Both looked like one, and there were five of them.
 *
 * `markAllStalledOnBoot` only rewrites rows that were *running*, which is right for what it
 * does and leaves everything else carrying whatever it happened to say when the process died.
 * A status line is a claim about now; after a restart most of them are claims about a world
 * that no longer exists.
 *
 * So each teammate is asked one question: is there live work waiting for you?
 *
 *   - **Yes** — an active step on a ticket that still exists. Left `stalled`, which is exactly
 *     what `resumeInterruptedWork` looks for, and it runs moments later.
 *   - **No** — set `idle` and the line cleared. Nothing is lost: the branch, the commits and
 *     the message log are all still there. What goes is a sentence that was no longer true.
 *
 * Deliberately not touching agents that are genuinely mid-something: this runs at boot, when
 * by definition nothing of ours is running yet.
 */
export function settleAgentsOnBoot(): number {
  let settled = 0

  for (const project of listProjects()) {
    const live = listTickets(project.id).filter((t) => !t.archivedAt)
    const wanted = new Set<string>()
    for (const ticket of live) {
      if (ticket.lane === 'done' || ticket.readyToMerge) continue
      const step = activeStep(acceptedRoute(ticket.id))
      if (step?.assigneeAgentId) wanted.add(step.assigneeAgentId)
      if (ticket.assigneeAgentId) wanted.add(ticket.assigneeAgentId)
    }

    for (const who of listAgents(project.id)) {
      if (who.isPilot) continue
      if (wanted.has(who.id)) continue
      if (who.status !== 'error' && who.status !== 'stalled') continue

      setAgentStatus(who.id, 'idle', null)
      settled++
    }
    if (settled > 0) bus.emitDomain({ type: 'agents:changed', projectId: project.id })
  }

  if (settled > 0) flushWrites()
  return settled
}
