...
/**
 * Is this agent waiting to start, as opposed to stalled?
 *
 * The board needs the distinction: queued means the wait ends by itself, stalled means it
 * never will. Without this they look identical from the outside, which is how a dropped
 * launch passed for a busy reviewer for hours.
 */
export function isQueued(agentId: string): boolean {
  return started.has(agentId) || queue.some((p) => p.agentId === agentId)
}
...
function whyWaiting(projectId: string, agentId: string): string {
  ...
  if (spendBlocked(projectId)) return 'Held — the project is at its spend ceiling'
...
function canStart(projectId: string, agentId: string): boolean {
  ...
  /*
   * The ceiling is enforced here rather than at each call site, because "here" is the one
   * place every launch passes through. It was previously enforced nowhere: stored since
   * migration 016, validated on the way in, read by nothing. A limit that is only written
   * down is not a limit.
   *
   * Unlike the cap, this wait does not end on its own — someone has to raise the ceiling or
   * reset the counters. It parks rather than fails so that doing either releases the work.
   */
  if (spendBlocked(projectId)) return false
