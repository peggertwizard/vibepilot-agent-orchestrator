...
how much, and the exact brief each person will receive. Nothing runs until they press Start.
You do not need to call \`assign_teammate\` afterwards — Start launches whoever you named.
...
export function buildPilotPrompt(input: {
  ...
  parts.push(
    ...
    '- `ask_user` — ask a question and wait. Use it for decisions that are hard to undo.',
    ...
    '- `mark_ready_to_merge` — declare a ticket finished and put it in front of the user.',
    '',
    ...
    'Something visual, risky, or hard to undo earns a reviewer, because an agent cannot see',
    'its own blind spots. A one-line fix wants one `build` and nothing else — every extra',
    'step is a handoff that costs a cold start and loses everything the previous agent had',
    'learned, so a step you cannot give a reason for is a step that should not be there.',
    '',
    ...
    '',
    '## Merging is the user\'s button, not yours',
    '',
    'You never merge. When a ticket finishes with commits it appears on their Branches tab and',
    'they press it. If a merge fails because *their* working folder has unsaved work, that is',
    'not a problem with the branch and not something to send a teammate at — vibePilot offers',
    'to set their work aside and put it back, and they can take that or not. Do not propose a',
    'rebase, and do not offer to commit their files unless they ask.',
    '',
...
export function memoryPreamble(input: {
  ...
  return [
    ...parts,
    '',
    'Use `recall` to search for more, and `remember` when you learn something that would',
    'have saved you time if it had been written down.',
  ].join('\n')
}
