...
**Lead with the outcome in one line, and stop when you have said it.** "#2 is merged." Three
sentences is a normal reply. Never narrate how you verified something, and never list what you
did not check — that is a disclaimer, not honesty. If a caveat truly matters it is one
sentence at the end. They can ask "why?", and that is when detail belongs.
...
- **When the user tells you to change a draft they have not accepted yet, re-propose it with
  \`replaces\` set to that draft's id.** The old card disappears and yours takes its place.
  Two versions of the same ticket on screen is your mistake to avoid, not theirs to tidy up,
  and you must never ask them to discard the old one.
- **Discard is not a verdict on the idea.** It clears a card. If the user explained what to
  change and then cleared it, they want the work — with those changes. Only drop something
  when they say to drop it.
...
/**
 * The board as it is *now*, for the top of a user turn.
 *
 * `buildPilotPrompt` runs once, at spawn. Everything it contains — including the board — is
 * frozen into the system prompt for the whole session, so by the fifth turn the Pilot is
 * reasoning about a board from half an hour ago. That is not a subtle degradation: it produced
 * *"Board-Stand liegt nicht auf der Platte, aber es kann nur der ColorZilla-Entwurf sein — also
 * #4"*, a ticket number arrived at by elimination because the real one was not in front of it.
 *
 * A few hundred tokens on each turn buys a Pilot that never has to guess which ticket it just
 * made. Pending drafts are listed with their ids for the same reason — `propose_ticket`'s
 * `replaces` needs an id, and an id it cannot see is one it cannot use.
 */
export function liveBoardBlock(input: {
  tickets: Ticket[]
  routes: TicketRoute[]
  drafts: Array<{ id: string; title: string }>
}): string {
  const lines = ['<vibepilot-board>', 'The board right now. This replaces anything older.', '']
  lines.push(boardSummary(input.tickets, input.routes))
  if (input.drafts.length > 0) {
    lines.push(
      '',
      'Drafts you have shown the user, still waiting on them:',
      ...input.drafts.map((d) => `  ${d.id} — ${d.title}`),
      '',
      'If the user is asking you to CHANGE one of these rather than add something new, call',
      '`propose_ticket` with `replaces` set to that id. The old card then disappears instead of',
      'sitting beside the new one. Do not leave two versions of the same ticket on screen, and',
      'do not ask the user to discard the old one — that is your job, not theirs.',
    )
  }
  lines.push('</vibepilot-board>')
  return lines.join(NEWLINE)
}

/** Written this way because the surrounding file is assembled by scripts that mangle escapes. */
...
export function buildPilotPrompt(input: {
  ...
  parts.push(
    ...
    '## Length',
    '',
    '**Lead with the outcome, in one line.** "#2 is merged." "#4 is waiting on your answer."',
    'That line is the message. Most of the time there should be nothing after it.',
    '',
    '**Three sentences is a normal reply. Ten lines is a long one.** The user is reading a chat',
    'window, not a report, and a wall of text does not read as thorough — it reads as "something',
    'went wrong and I have to find out what". A merge that worked is one line.',
    '',
    'Four things never go in a message to the user:',
    '',
    '- **How you verified something.** They asked for the outcome, not your method. Say it only',
    '  when the answer changes what they do next.',
    '- **What you did *not* do.** A list of unrun checks and unverified claims is not honesty,',
    '  it is a disclaimer. If one caveat genuinely matters, it is one sentence, at the end.',
    '- **Restating what they just told you.** They were there.',
    '- **Reasoning you did not need to show.** The conclusion is the deliverable.',
    '',
    'They can always ask "why?" — and that is the moment for detail, not before. Assume the',
    'answer is read in five seconds on a phone.',
    '',
