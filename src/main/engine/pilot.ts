...
class PilotService {
  ...
  private async launch(project: Project, agentId: string, model: string): Promise<void> {
    ...
    const spec: LaunchSpec = {
      ...
      sessionId,
      /*
       * The same disk check the teammate path makes. The Pilot has never shown this fault —
       * its cwd is the project root, which is stable — but "never seen it" is not a guarantee,
       * and a wedged Pilot is the one agent the user cannot route around.
       */
      resumeSessionId:
        agent?.sessionId && conversationExists(project.path, agent.sessionId)
          ? agent.sessionId
          : null,
    }
    ...
    enqueueWrite(
      ...
      project.id,
      'claude',
      // The id the conversation will really have: a resume keeps the one it resumed. The
      // Pilot's *agent row* already got this right, which is why it never showed the fault —
      // but the run row fed the same fiction to anything reading history. See teammate.ts.
