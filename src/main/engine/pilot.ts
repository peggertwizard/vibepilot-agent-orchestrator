...
import { buildPilotPrompt, ensureProjectConfig } from './context'
...
class PilotService {
  ...
  async send(
    ...
    const agentId = await this.ensure(projectId, model)
  ...
  notify(projectId: string, body: string): void {
    const pilot = getPilot(projectId)
    if (!pilot) return
    manager.send(pilot.id, {
