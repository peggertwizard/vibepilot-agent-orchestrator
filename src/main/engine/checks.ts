...
/**
 * What the checks said, per agent, for the life of this process.
 *
 * Lived as a private map in `mcp/tools.ts` when the only reader was the tool that attached
 * check output to a ready-report. Auto-merge is a second reader — it will not land work whose
 * checks failed — and two copies of "what did the checks say" would be one copy too many.
 *
 * In memory on purpose: the question is "did they pass *on this run*", and a restart means
 * nobody has run anything yet.
 */
const lastChecks = new Map<string, CheckResult[]>()

export function recordChecks(agentId: string, results: CheckResult[]): void {
  lastChecks.set(agentId, results)
}

/** Null means "nothing was run", which is not the same as "they failed". */
export function lastChecksFor(agentId: string | null): CheckResult[] | null {
  if (!agentId) return null
  return lastChecks.get(agentId) ?? null
}
