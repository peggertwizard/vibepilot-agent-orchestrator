...
class Bootstrap {
  ...
  async scan(projectId: string, timeoutMs = 180_000): Promise<boolean> {
    ...
    /*
     * Say it started, before it starts.
     *
     * This spawns a real Claude process and reads the repository — a minute is normal, and the
     * whole minute produced nothing at all: no message, no status, no row anywhere. The scan
     * agent is deliberately kept off the board (`bootstrap:` ids are filtered out of the live
     * buffer), which is right, and left the only visible sign of it being the moment it
     * finished. *"it took long and there was no indication that i have to wait."*
     */
    addMessage({
      projectId,
      authorType: 'agent',
      kind: 'notice',
      body:
        'Reading the project to work out who should be on the team — files, structure, ' +
        'what it is built with. This takes a minute or two, and nothing is changed: the scan ' +
        'can only read.',
    })
    flushWrites()
    ...
    if (!parsed?.team?.length) {
      /*
       * A failed parse no longer burns the offer.
       *
       * It used to stamp `bootstrapped_at` before checking, on the reasoning that a scan
       * silently retrying on every launch is worse than one that gives up. That reasoning
       * belonged to `maybeScan`, which nothing calls any more — the scan is a button now, so
       * "it retries every launch" is not a thing that can happen, and a scan that failed to
       * parse its own output should not permanently remove the way to try again.
       */
      addMessage({
        projectId,
        authorType: 'agent',
    ...
    // It worked, so the one-time offer has served its purpose. The scan itself stays available
    // from the Team tab — this only retires the first-run card.
    this.markDone(projectId)
  ...
  private markDone(projectId: string): void {
    ...
    /*
     * Announced, so the offer card disappears because the *data* changed rather than because
     * a component remembered it had been pressed. Local state was doing that job, and local
     * state in an unkeyed pane is shared between projects — dismissing the offer on one
     * project silently swallowed the other project's.
     */
    flushWrites()
    bus.emitDomain({ type: 'projects:changed', projectId })
  }
}
