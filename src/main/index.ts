import { BrowserWindow, app, powerMonitor, session, shell } from 'electron'
...
import { resumeInterruptedWork, settleAgentsOnBoot } from './engine/heal'
...
app.whenReady().then(() => {
  ...
  /*
   * Coming back from sleep is not a failure, but from in here it looks exactly like one.
   *
   * Connections to the model are gone, every timer that fell due while the lid was shut fires
   * at once, and processes Windows helpfully preserved are holding sockets that will never
   * answer again. The heartbeat finds most of that eventually — up to three minutes later, and
   * only once per teammate per app run, which a hibernation should not be allowed to spend.
   *
   * So check straight away, with everyone's automatic attempt reset. Registered after the
   * heartbeat so there is something to reset.
   */
  powerMonitor.on('resume', () => resumeInterruptedWork())

  /*
   * And the same for a plain restart.
   *
   * `markAllStalledOnBoot` above marks everything that was running as stalled, which was the
   * whole story: the board showed a Restart button and waited for a person to press it. But
   * the session id and the worktree are persisted precisely so a resume can carry on, and
   * nothing about closing the app is a decision the user needs to re-make on the way back in.
   *
   * After `createWindow` so the messages it writes land in a UI that exists, and behind a
   * short delay so the first paint is not competing with a spawn.
   */
  /*
   * And make the rest of the roster stop claiming things that stopped being true.
   *
   * Before the resume, so a teammate that genuinely has work waiting is still `stalled` when
   * `resumeInterruptedWork` goes looking, and everyone else has already been quietened.
   */
  setTimeout(() => {
    const settled = settleAgentsOnBoot()
    if (settled > 0) console.log(`[boot] cleared stale status on ${settled} agent(s)`)
    if (stalled > 0) resumeInterruptedWork()
  }, 3_000).unref?.()
