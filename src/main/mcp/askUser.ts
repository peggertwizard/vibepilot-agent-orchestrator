...
import { CODEX_CAPS } from '../providers/codex/adapter'
import { CLAUDE_CAPS } from '../providers/types'
import type { RunBinding, ToolResult } from './tools'

/**
 * Can this agent's process sit and wait for a human?
 *
 * Read off the provider's own capability flag rather than its name, which is the rule the whole
 * provider layer is built on: `persistentTurns` is exactly the question "does one process span
 * many turns", and it is what decides whether blocking is patience or a lie.
 */
function providerHoldsTurn(agentId: string): boolean {
  const provider = getAgent(agentId)?.provider
  return (provider === 'codex' ? CODEX_CAPS : CLAUDE_CAPS).persistentTurns
}
...
class AskUserGate {
  ...
  async wait(questionId: string, b: RunBinding, isContinuation = false): Promise<ToolResult> {
    ...
    /*
     * A teammate that cannot wait must not be made to.
     *
     * Blocking here is right for Claude: one process spans many turns, so holding the tool call
     * open costs nothing and the answer arrives inside the same session with its context warm.
     *
     * Codex is the opposite shape. `codex exec` runs one turn and exits, so there is no session
     * sitting there to receive anything — and its MCP client kills a tool call long before a
     * person has read the question. What that produced was a Writer reporting *"ich warte nicht
     * weiter auf eine nicht sichtbare Tool-Abfrage"*: it had asked, the question was on screen
     * in front of the user, and from inside the process the call had simply failed. It then
     * carried on guessing, which is the precise outcome asking exists to prevent.
     *
     * So for a provider with no persistent turns the question is posted and the call returns at
     * once. The turn ends cleanly, the card is waiting for the user, and `wakeAsker` resumes the
     * thread with the answer the moment it arrives — machinery that already exists and already
     * runs. Nothing is lost but the pretence of waiting.
     */
    if (!providerHoldsTurn(b.agentId)) {
      return {
        content: [
          {
            type: 'text',
            text:
              'Asked. The question is in front of the user now. ' +
              '**Do not wait for it and do ' +
              'not call `await_answer`** — your turn ends here, and you will be started again ' +
              'with the answer as soon as they reply, with this thread and everything you have ' +
              'already done intact. Finish the turn by saying briefly what you are waiting on.',
          },
        ],
        structuredContent: { ok: true, status: 'pending', question_id: questionId, resume: 'automatic' },
      }
    }
