...
import { renderChecks, runChecks } from '../engine/checks'
import { bus } from '../bus'
import {
  findAgentByName,
  getAgent,
 ...
 */
const lastChecks = new Map<string, CheckResult[]>()
...
export const TOOL_DEFS: ToolDef[] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        size_note: { type: 'string', description: 'Rough size, e.g. "half a day".' },
    ...
    run: (raw, b) => {
      ...
      return okResult(
        ...
        { draft_id: draft.id, status: 'awaiting_confirmation' },
      )
    },
  },
...

  {
    ...
    description:
      ...
      'planning and a reviewer; a visual change earns a review because a builder cannot see ' +
      'its own blind spots. Equally, do not add a step you cannot give a reason for — every ' +
      'handoff costs a cold start and loses what the last agent learned. Worked examples:\n' +
      '  "Fix the typo in the footer"         -> [build]\n' +
      '  "Which file handles session expiry?" -> [research]            (no code at all)\n' +
      '  "Add a dark mode toggle"             -> [build, review]       (it is visual)\n' +
      '  "Migrate auth to the new provider"   -> [plan, build, review] (hard to undo)\n\n' +
...

  {
    ...
    run: async (raw, b) => {
      ...
      if (out.routeComplete) {
        ...
        if (commits > 0) {
          updateTicket(t.id, { readyToMerge: true, mergeState: 'ready', lane: 'in_progress' })
          addMessage({
            projectId: b.projectId,
            agentId: b.agentId,
            authorType: 'system',
...

  {
    ...
    run: (raw, b) => {
      ...
      const limit = getProject(b.projectId)?.reviewPasses ?? MAX_REVIEW_PASSES
...
This is flagged as a blocker. Decide whether it changes what anyone ` +
...

  {
    ...
    run: async (_raw, b) => {
      ...
      lastChecks.set(b.agentId, results)
...

  {
    ...
    run: (raw, b) => {
      ...
      const ran = lastChecks.get(b.agentId)
      let verdict = ''
...

  {
    ...
    run: async (raw, b) => {
      ...
      // construction.
      const busy = manager.forAgent(who.id)
      if (busy && who.currentTicketId !== t.id) {
        ...
        const free = listAgents(b.projectId)
          .filter((x) => !x.isPilot && x.isRoster && !manager.forAgent(x.id))
          .map((x) => x.name)
        return deny(
