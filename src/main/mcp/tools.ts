...
This is flagged as a blocker. Decide whether it changes what anyone ` +
...

  {
    ...
    run: async (raw, b) => {
      ...
      if (proj && t.branch) {
        const ahead = await commitsAhead(proj.path, proj.defaultBaseBranch, t.branch)
        if (ahead === 0) {
          return deny(
            `\`${t.branch}\` has no commits on it, so there is nothing to merge. Your work is ` +
              `not where vibePilot is looking.\n\n` +
...

  {
    ...
    run: (raw, b) => {
      ...
      setAgentStatus(who.id, 'queued', 'Restarting a stalled step')
      flushWrites()
      bus.emitDomain({ type: 'agents:changed', projectId: b.projectId })
      ...
      gate.submit({
        ...
        run: () =>
          launchTeammate({
            ...
            brief:
              step.brief ??
              `Carry on with #${t.number}: ${t.title}. Check what you have already done ` +
                `before redoing any of it.`,
            pilotAgentId: b.agentId,
            /*
             * The point of the whole tool. A cold start would re-read the codebase and repeat
             * work already sitting committed in the worktree — which is why the session id and
             * the worktree have been persisted before spawn since the process layer was built.
             */
