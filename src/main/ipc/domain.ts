...
export function registerDomainIpc(handle: Handle): void {
  ...
  handle(
    ...
    ({ agentId }) => {
      ...
      setAgentStatus(agentId, 'queued', 'Restarting')
      flushWrites()
      ...
      gate.submit({
        ...
        run: () =>
          launchTeammate({
            ...
            // The whole point. A cold start here would re-read the codebase and repeat work
            // that is already sitting committed in the worktree.
