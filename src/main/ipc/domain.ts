...
import { git, hasLanded } from '../git/repo'
...
import { clearHealAttempts, relaunchAssignee, wakeAsker } from '../engine/heal'
...
/**
 * Bytes on disk, best effort — and never at the cost of the window.
 *
 * This was a synchronous recursive walk with a `statSync` per file, called once per worktree
 * every time the Branches tab loaded. A worktree of a real project is a full checkout plus
 * `node_modules`: forty to sixty thousand files each. Three of them is a couple of hundred
 * thousand blocking syscalls inside one IPC handler, on the main process, with the event loop
 * stopped for all of it — which is precisely what Windows puts *"vibePilot is not responding"*
 * on the title bar for. Nothing was wrong; it was counting.
 *
 * Three changes, in order of how much they matter:
 *
 *   - **Asynchronous.** `fs/promises` yields between directories, so the window keeps painting
 *     and every other IPC call still gets answered while this runs.
 *   - **Capped.** A `node_modules` tree is most of the count and none of the interest. Past
 *     `MAX_ENTRIES` it stops and reports what it has — the number exists to answer "is it
 *     worth deleting", and "at least 400 MB" answers that as well as an exact figure.
 *   - **Cached.** The tab reloads on mount and after every merge, push and removal. A worktree
 *     does not change size between two clicks.
 */
...
async function dirSize(path: string): Promise<number> {
  ...
  let total = 0
  let seen = 0
  // Breadth-first with an explicit queue rather than recursion: the depth cap becomes a
  // by-product of the entry cap, and there is no stack to overflow on a pathological tree.
  ...
  sizeCache.set(path, { bytes: total, at: Date.now() })
  return total
}

/** A worktree that is gone has no size, and its old number must not outlive it. */
function forgetSize(path: string): void {
...
export function registerDomainIpc(handle: Handle): void {
  ...
  handle(
    ...
    async ({ projectId, ticketId }) => {
      /*
       * Stop whoever is actually on it — here, not in the renderer.
       *
       * The Cancel button stopped `ticket.assigneeAgentId`, which is frequently not the agent
       * doing the work: a route puts its assignee on the *step*, so cancelling a ticket mid-plan
       * stopped nobody and the teammate carried on against a card that had left the board.
       * Doing it in the main process means every path that archives — the button, the sweep,
       * the Pilot — gets the same behaviour, instead of one of them remembering.
       */
      const step = activeStep(acceptedRoute(ticketId))
      const ticket = getTicket(ticketId)
      const workers = new Set(
        [ticket?.assigneeAgentId, step?.assigneeAgentId].filter((x): x is string => !!x),
      )
      for (const agentId of workers) {
        // Freed regardless of whether one was running, so heal cannot bring it back either.
        clearHealAttempts(agentId)
        if (manager.forAgent(agentId)) {
          await manager.stop(agentId, 'its ticket was cancelled').catch(() => undefined)
        }
      }
      ...
      flushWrites()
      bus.emitDomain({ type: 'tickets:changed', projectId })
      if (workers.size > 0) bus.emitDomain({ type: 'agents:changed', projectId })
      return true
    },
  )
  ...
  handle(
    ...
    ({ projectId, draftId, reason }) => {
      ...
      bus.emitDomain({ type: 'drafts:changed', projectId })
      /*
       * What Discard actually means, which is less than it used to claim.
       *
       * This said "Do not re-propose the same thing." That is one reading of pressing Discard,
       * and it was applied to every reading. A user who types the corrections they want and
       * then clears the stale card has not turned the idea down — but the Pilot was told they
       * had, twice in a row, and concluded it was guessing wrong: *"Zwei Ablehnungen
       * hintereinander heißt, ich rate falsch. Ich höre auf zu raten."* It then stopped
       * proposing and started asking. Nothing had been rejected; the app said so on the user's
       * behalf.
       *
       * The card is gone and that is the whole of what is known. What the user wants next is in
       * what they typed, which the Pilot can already read.
       */
      pilot.notify(
        projectId,
        `The user cleared the draft "${d?.title ?? 'that draft'}" from their screen.` +
          (reason ? ` They said: ${reason}` : '') +
          ` That removes the card; it does not tell you whether they still want the work. If ` +
          `they have said what to change, act on that. Only treat the idea as dropped if they ` +
          `actually said so.`,
      )
      return true
    },
   ...
   */
  /**
   * Forget everything this teammate is carrying and start it clean.
   *
   * *"why can't we just reset the senior dev?"* — and there was no answer, which was the
   * problem. Every wedged agent this week ended the same way: some stored state was wrong,
   * nothing on screen could clear it, and the remedy was for somebody to go and find the bad
   * row. This does not need to know which piece went bad, because it drops all of them.
   *
   * The worktree, the branch and the commits are untouched — that is the work. What a reset
   * costs is the conversation: the teammate re-reads what it already did rather than
   * remembering it. That is why it is safe to reach for first.
   *
   * Stops it first if something is genuinely running, then starts it again on its ticket if it
   * has one, so the button is a single act rather than reset-then-remember-to-restart.
   */
  handle('agents:reset', z.object({ agentId: z.string().min(1) }), async ({ agentId }) => {
    const who = getAgent(agentId)
    if (!who) return { ok: false as const, reason: 'That teammate no longer exists.' }

    if (manager.forAgent(agentId)) {
      await manager.stop(agentId, 'reset by the user').catch(() => undefined)
    }
    resetAgent(agentId)
    // A fresh start deserves a fresh automatic attempt, whatever the cooldown was holding.
    clearHealAttempts(agentId)
    flushWrites()
    bus.emitDomain({ type: 'agents:changed', projectId: who.projectId })

    const ticket = listTickets(who.projectId).find(
      (t) =>
        t.assigneeAgentId === agentId ||
        activeStep(acceptedRoute(t.id))?.assigneeAgentId === agentId,
    )
    ...
    addMessage({
      ...
      kind: 'notice',
      body:
        `${who.name} was reset — its saved conversation is gone and it starts fresh. ` +
        `Its branch and everything it had committed are untouched; it will read them rather ` +
        `than remember them.` + (ticket ? ` Starting again on #${ticket.number}.` : ''),
    })
    flushWrites()
    bus.emitDomain({ type: 'messages:changed', projectId: who.projectId })

    if (!ticket) return { ok: true as const, started: false }
    ...
    const started = relaunchAssignee({
      ...
      brief:
        step?.brief ??
        `Carry on with #${ticket.number}: ${ticket.title}. You are starting fresh — read the ` +
          `branch to see what has already been done before redoing any of it.`,
      because: 'Reset and starting over',
  ...
  handle(
    ...
    ({ projectId, agentId, text }) => {
      ...
      let sent = manager.send(agentId, { text, channel: 'user' })

      /*
       * "Not running" is a normal resting state for half the providers.
       *
       * Claude holds one process open across many turns, so no process means something died.
       * Codex does not: `codex exec` runs a turn and exits, so a Codex teammate is *usually*
       * not running — between turns, and in particular right after it has asked a question and
       * finished its turn waiting for the answer.
       *
       * This threw on that, and the drawer read `LIVE_STATUSES` and greyed the composer with
       * *"Writer isn't running — say it on the ticket instead."* The teammate had asked
       * something, the user had the answer, and the app said there was nobody to tell. The
       * ticket sat there.
       *
       * A teammate with a ticket and a session is reachable whether or not a process happens to
       * be alive: `relaunchAssignee` resumes it in its own worktree with its context intact,
       * which is the same path the heal pass and `wakeAsker` already use.
       */
      if (!sent) {
        const ticketId =
          listTickets(projectId).find((t) => t.assigneeAgentId === who.id)?.id ??
          listTickets(projectId).find(
            (t) => activeStep(acceptedRoute(t.id))?.assigneeAgentId === who.id,
          )?.id ??
          null

        if (ticketId) {
          sent = relaunchAssignee({
            agentId: who.id,
            ticketId,
            brief:
              `The user says:
...
Carry on from where you were. Check what you had ` +
              `already done before redoing any of it.`,
            because: 'Picking up what you said',
          })
        }
      }
      ...
      if (!sent) {
        throw new Error(
          `${who.name} is not running and has no ticket to resume, so there is nobody to tell.`,
        )
      }
  ...
  handle(
    ...
    ({ projectId, questionId, answer }) => {
      ...
      const delivered = askUserGate.deliver(questionId, answer, 'user')
      /*
       * Nobody was waiting — so wake whoever asked.
       *
       * The return value was ignored, and `false` is not rare: it is what happens whenever the
       * asking process is gone, which after a hibernation is every one of them. The answer was
       * written to the database, the card disappeared, and nothing anywhere consumed it.
       * *"I answer a few questions and after that nothing happened."*
       */
      if (!delivered) wakeAsker(questionId)
  ...
  /**
   * Delete the ticket branches whose work is already on the base.
   *
   * Nothing ever retired one, so they accumulated: ten in the panel on a real project, five of
   * them entirely dead. Harmless in git and expensive in attention — it is most of the reason
   * "what is actually left to merge" was impossible to read at a glance.
   *
   * Deliberately a button and never automatic. Deleting a branch is the one thing here that
   * cannot be undone from inside the app, and a landed-check that is ever wrong would take real
   * work with it. So the app hides them and the user decides.
   *
   * Three guards, each for a different way this could go wrong:
   *
   *   - **`hasLanded` is re-asked here**, not trusted from whatever the renderer was showing.
   *     A branch that gained a commit since the panel last drew is not deleted.
   *   - **the base branch and the current branch are never candidates**, whatever they are
   *     called — deleting the branch you are standing on is a state git will not even produce.
   *   - **a branch with a live worktree is left alone.** An agent may still be resumable there,
   *     and a Claude session is bound to its directory.
   */
  handle('git:deleteLandedBranches', ProjectId, async ({ projectId }) => {
    const project = getProject(projectId)
    if (!project) return { ok: false as const, reason: 'That project no longer exists.' }

    const here = await currentBranch(project.path)
    const busy = new Set(
      (await listWorktrees(project.path).catch(() => [])).map((w) => w.branch).filter(Boolean),
    )

    const { ticketBranches } = await overview(project.path, project.defaultBaseBranch)
    const deleted: string[] = []
    const kept: string[] = []

    for (const line of ticketBranches) {
      if (!line.landed) continue
      if (line.name === project.defaultBaseBranch || line.name === here) continue
      if (busy.has(line.name)) {
        kept.push(line.name)
        continue
      }
      // Asked again, against git, at the moment of deleting rather than the moment of drawing.
      if (!(await hasLanded(project.path, project.defaultBaseBranch, line.name))) {
        kept.push(line.name)
        continue
      }
      /*
       * `-D`, and only because `hasLanded` just said so. `-d` refuses anything git does not
       * consider merged, and a squash merge is precisely the case git does not recognise — so
       * the safe flag would refuse every branch this feature exists for. The content check is
       * the safety, and it is stricter than what `-d` would have asked.
       */
      try {
        await git(project.path, ['branch', '-D', line.name])
        deleted.push(line.name)
      } catch {
        kept.push(line.name)
      }
    }
    ...
    if (deleted.length > 0) {
      addMessage({
        ...
        kind: 'notice',
        body:
          `Deleted ${deleted.length} branch${deleted.length === 1 ? '' : 'es'} whose work is ` +
          `already on ${project.defaultBaseBranch}: ${deleted.join(', ')}. Nothing left ` +
          `${project.defaultBaseBranch} — every line of it is still there.`,
      })
      flushWrites()
      bus.emitDomain({ type: 'messages:changed', projectId })
      bus.emitDomain({ type: 'tickets:changed', projectId })
    }
    return { ok: true as const, deleted, kept }
  })

  /**
   * Put the project folder back on the base branch.
   *
   * The single repository fact everything else rests on, and the one the app could report but
   * never repair. Exposed as an action rather than done quietly: a checkout moves what a dev
   * server is serving and what the next teammate branches from, which is not a thing to do to
   * somebody without asking.
   */
  handle('git:checkoutBase', ProjectId, async ({ projectId }) => {
    const project = getProject(projectId)
    if (!project) return { ok: false as const, reason: 'That project no longer exists.' }
    ...
    if (r.ok) {
      addMessage({
        ...
        kind: 'notice',
        body:
          `Your project folder is back on ${project.defaultBaseBranch} (it was on ${r.from}). ` +
          `Anything already merged is what it shows now — a dev server watching this folder ` +
  ...
  /**
   * Land the branch the project folder is standing on.
   *
   * Work can end up on a branch with no ticket behind it — an agent that ran git in the main
   * folder, a hand checkout, a piece of setup. There was then no way to merge it from inside
   * the app at all: the merge button belongs to a ticket, and this branch has none. Same
   * `squashMerge` every ticket uses, so the same guarantees hold — conflicts stop, your own
   * changes stop it, and nothing is pushed.
   */
  handle('git:mergeCurrent', ProjectId, async ({ projectId }) => {
    const project = getProject(projectId)
    if (!project) return { ok: false as const, reason: 'That project no longer exists.' }

    const here = await currentBranch(project.path)
    if (!here) return { ok: false as const, reason: 'The folder is not on a branch.' }
    if (here === project.defaultBaseBranch) {
      return { ok: false as const, reason: `Already on ${project.defaultBaseBranch}.` }
    }
    ...
    addMessage({
      ...
      kind: 'notice',
      body:
        `\`${here}\` landed on ${project.defaultBaseBranch} as ${result.sha.slice(0, 7)}. ` +
        `Nothing was pushed. The folder is still on \`${here}\` — putting it back on ` +
        `${project.defaultBaseBranch} now hides nothing, because everything is in both places.`,
    })
    flushWrites()
    bus.emitDomain({ type: 'messages:changed', projectId })
  ...
  handle('git:worktrees', ProjectId, async ({ projectId }) => {
    ...
    // Sized concurrently: each walk is I/O-bound and yields, so a handful of worktrees costs
    // about as long as the largest one rather than the sum of all of them.
    return Promise.all(
      entries
  ...
  handle(
    ...
    async ({ projectId, path }) => {
      ...
      const freed = await dirSize(path)
      const result = await removeWorktree(project.path, path)
      if (result.removed) {
        // The cached size must not outlive the folder — the next listing would report a
        // worktree that is not there any more as still taking up disk.
        forgetSize(path)
  ...
  handle(
    ...
    async ({ projectId, ticketId, setAside }) => {
      ...
      let freed = ''
      if (ticket.worktreePath) {
        const bytes = await dirSize(ticket.worktreePath)
        const removal = await removeWorktree(project.path, ticket.worktreePath)
        if (removal.removed) {
          forgetSize(ticket.worktreePath)
          await pruneWorktrees(project.path).catch(() => undefined)
