...
import { clearResolveCache, resolveClaude, resolveCodex, probeVersion } from '../providers/process/resolve'
import { clearCodexModelCache, codexModels } from '../providers/codex/models'
...
export function registerIpc(getWindow: GetWindow): void {
  ...
  /**
   * Which Codex models this machine has.
   *
   * Asked of the installed CLI rather than answered from a list in vibePilot — see
   * `providers/codex/models.ts` for why that is the only version of this that cannot go stale.
   * An empty array is a real answer (no Codex, or a version too old to be asked) and the
   * teammate editor falls back to a free-text field on it.
   */
  handle('codex:models', z.object({}).optional(), () => codexModels())

  /** Where the Codex binary is, when the ladder cannot find it. Mirrors the Claude pair. */
  handle('settings:codexBinary', z.object({}).optional(), () => getSetting('codexBinaryPath'))
  handle('settings:setCodexBinary', z.object({ path: z.string().max(1000) }), ({ path }) => {
    setSetting('codexBinaryPath', path.trim())
    clearResolveCache()
    // The model list belongs to the binary it was read from. A new path is a new answer.
  ...
  handle('system:doctor', Empty, async (): Promise<DoctorReport> => {
    ...
    /*
     * Codex is optional, so nothing here is a problem — it is reported so that its absence is
     * visible instead of showing up later as an empty model picker with no explanation.
     */
    const codex = await resolveCodex()
    const codexVersion = codex ? await probeVersion(codex) : null
    const codexModelCount = codex ? (await codexModels().catch(() => [])).length : null
    ...
    return {
      ...
      codexBinary: codex?.file ?? null,
      codexVersion,
      codexModels: codexModelCount,
      gitVersion: gv,
      ghVersion: gh,
