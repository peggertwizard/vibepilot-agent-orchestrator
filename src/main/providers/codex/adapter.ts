...
export class CodexCliAdapter implements ProviderAdapter {
  ...
  /*
   * The running totals Codex last reported, so they can be turned back into per-turn numbers.
   *
   * `turn.completed.usage` is cumulative **for the whole thread**, not for the turn — measured:
   * a trivial two-turn thread reported input 19,051 then 39,686, and output 5 then 10. Every
   * consumer downstream adds what it is given, so each turn re-added everything before it and
   * the totals grew quadratically. A single text ticket showed **2.65M tokens** and a context
   * reading of **1.39M**, which is not a large number, it is a wrong one.
   *
   * Held on the adapter rather than the translator because the translator is rebuilt for every
   * turn (a turn is a process here) and would have nothing to compare against.
   */
  private reported = { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }
  ...
  private onWire(v: unknown): void {
    ...
    for (const e of events) {
      ...
      this.emit(e.type === 'agent:cost' ? this.asDelta(e) : e)
    }
  ...
  /**
   * Cumulative in, per-turn out.
   *
   * Both numbers this produces are then honest and mean different things. The **deltas** are
   * what this turn actually cost, which is what a total should be built from. And the input
   * delta is also the size of everything sent on this turn — which, since Codex re-uploads the
   * whole conversation each time, *is* the context in use. So the same subtraction fixes the
   * running total and gives the context reading its real value.
   *
   * A counter that goes backwards means a new thread, or a CLI that changed its mind about what
   * this field is. Trust the smaller number and start again rather than emit a negative.
   */
  private asDelta(e: AgentEvent & { type: 'agent:cost' }): AgentEvent {
    ...
    const stepped = cur.input >= prev.input && cur.output >= prev.output
    const base = stepped ? prev : { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }
    this.reported = cur

    const sent = Math.max(0, cur.input - base.input)
    return {
      ...e,
      inputTokens: sent,
      outputTokens: Math.max(0, cur.output - base.output),
      cacheReadTokens: Math.max(0, cur.cacheRead - base.cacheRead),
      cacheCreationTokens: Math.max(0, cur.cacheWrite - base.cacheWrite),
      contextUsed: sent > 0 ? sent : undefined,
    }
  }
