  ...
  return [
    ...
    ...(spec.mcp
      ? [
          ...
          /*
           * Long enough for a person to answer.
           *
           * Codex's default per-tool timeout is a minute or so, and `ask_user` waits for a
           * human. So a Codex teammate that asked a question had its own tool call killed
           * underneath it and reported *"ich warte nicht weiter auf eine nicht sichtbare
           * Tool-Abfrage"* — it had asked, the question was on screen, and from inside the
           * process the call had simply failed. Same ten minutes the Claude side sets, and for
           * the same reason.
           *
           * Belt and braces only: `askUser` no longer blocks a spawn-per-turn provider at all
           * (see `SOFT_TIMEOUT_MS` there). This covers every *other* tool, and covers the case
           * where that provider check is ever wrong.
           */
          '-c',
