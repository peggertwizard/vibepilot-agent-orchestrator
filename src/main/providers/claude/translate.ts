  ...
  switch (m['type']) {
    ...
    case 'result': {
      ...
      if (m['terminal_reason'] === 'budget_exhausted') {
        ...
        /*
         * The reason is usually in `errors`, and this branch never read it.
         *
         * The budget branch above has always read `m['errors']`; this one fell back to the
         * literal "The model call failed." whenever `result` was empty — which it is precisely
         * when something went wrong early. So the one string the CLI provided to explain
         * itself was dropped, and what reached the board, the Pilot and the user was a
         * sentence with no information in it: *"still immediately says: Senior Dev (#5)
         * stopped: The model call failed."* Nothing anywhere could say *what* failed, so
         * nobody — including the retry logic — could tell a rate limit from a dead API key.
         */
        const errs = m['errors']
        const detail =
          (typeof m['result'] === 'string' && m['result']) ||
          (Array.isArray(errs) && typeof errs[0] === 'string' && errs[0]) ||
          ''
        out.push({
          ...base(),
          type: 'agent:error',
          ...
          message:
            detail ||
            `The model call failed${m['api_error_status'] ? ` (HTTP ${String(m['api_error_status'])})` : ''}, and the CLI gave no detail.`,
          recoverable: true,
        })
      } else {
        out.push({
