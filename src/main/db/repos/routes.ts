import type { EffortLevel, RouteStatus, RouteStep, StepKind, TicketRoute } from '@shared/types'
...
export function unroutedTickets(
  projectId: string,
): Array<{ id: string; number: number; title: string; needsPlanning: boolean }> {
  return all<{ id: string; number: number; title: string; needs_planning: number }>(
    `SELECT t.id, t.number, t.title, t.needs_planning FROM tickets t
     WHERE t.project_id = ?
    ...
    projectId,
    // `needs_planning` comes along because the routing prompt is the only place it can
    // matter: it is the user saying "let me read the plan before anything is built", and it
    // has to reach the turn that chooses the shape of the route.
  ).map((r) => ({ id: r.id, number: r.number, title: r.title, needsPlanning: r.needs_planning === 1 }))
}
...
export interface StepSpec {
  ...
  gate?: boolean
  /** Per-step overrides. Null means "whatever the teammate is set to". */
  model?: string | null
...
export interface AdvanceResult {
  ...
  /**
   * The next step needs the user's sign-off, so nothing was started.
   *
   * Distinct from `routeComplete`: the work is half done and paused on purpose, not finished.
   * Conflating them would mark the ticket ready and try to merge a plan document.
   */
  gated: boolean
}
...
export function completeActiveStep(ticketId: string): AdvanceResult | null {
  return tx(() => {
    ...
    /*
     * A gated step does not start itself.
     *
     * This used to advance unconditionally, which is why "plan it, then I approve, then build"
     * had no representation: the only way to stop before the build was to never start the
     * plan. Now the safe prefix runs and the route waits here, with the plan written and
     * readable — which is the whole reason to have planned first.
     */
    const gated = Boolean(next?.gate)
    if (next && !gated) next.status = 'active'
    ...
    return {
      ...
      next,
      /*
       * A gated route is NOT complete — the work is half done and waiting on a person. Saying
       * otherwise would mark the ticket ready and try to merge a plan document as if it were
       * the feature.
       */
...
/**
 * Approve the gated step and let the route carry on.
 *
 * Clears the gate rather than skipping it: the step still runs, it simply no longer waits.
 * Leaving the flag set would park the route again at the next advance, which is the sort of
 * thing that looks like the button not working.
 */
export function approveGate(ticketId: string): TicketRoute | null {
  return tx(() => {
    const r = acceptedRoute(ticketId)
    if (!r) return null

    /*
     * Only when the route has actually reached the gate.
     *
     * Without this you could approve a build while the plan that informs it is still running —
     * signing off on a document nobody has written. The gate is a *pause point*, and a pause
     * point you can press through early is not one.
     */
    if (activeStep(r)) return null

    const gated = r.steps.find((s) => s.status === 'pending' && s.gate)
    if (!gated) return null

    const steps = r.steps.map((s) =>
      s.id === gated.id ? { ...s, gate: false, status: 'active' as const } : s,
    )
    run(
      'UPDATE ticket_routes SET steps_json = ?, updated_at = ? WHERE id = ?',
      json(steps),
...
/**
 * The steps that may run before anyone is asked anything.
 *
 * Research and plan are safe: they read, think, and write a document into a worktree. Nothing
 * reaches the base branch and nothing deploys. So a route that ends in a gate can still get
 * moving — and by the time it needs a decision, there is a plan to make it with.
 */
export function safePrefixLength(route: TicketRoute): number {
  let n = 0
