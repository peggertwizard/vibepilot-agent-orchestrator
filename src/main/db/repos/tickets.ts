...
export function resolveDraft(
  draftId: string,
  /**
   * `superseded` is the one that is not an ending for the *idea*.
   *
   * The user asked for a change and this is the new version. Distinguished from `rejected`
   * because the two produce opposite messages to the Pilot — one says "drop it", the other says
   * nothing at all, because the replacement is already on screen.
   */
  status: 'created' | 'parked' | 'rejected' | 'superseded',
