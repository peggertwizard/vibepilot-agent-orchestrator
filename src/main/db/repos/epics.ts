...
export function proposeSplit(input: {
  ...
  /*
   * One open proposal per project. A new one supersedes the old.
   *
   * A split proposal used to sit on the board until it was explicitly accepted or dropped. So
   * you could talk the Pilot round to a different shape, get a second proposal, and have two
   * breakdowns of the same request on screen — either of which could be confirmed, and both of
   * which would create tickets. That is a duplicate-ticket machine sitting where the board is
   * supposed to be telling you what is happening.
   *
   * Superseded rather than deleted: the Pilot's earlier thinking stays inspectable, it just
   * stops being something you can accidentally accept.
   */
  run(
    "UPDATE epics SET status = 'superseded', updated_at = ? WHERE project_id = ? AND status = 'proposed'",
    t,
    input.projectId,
  )
...
export function unmetDependencies(projectId: string, ticketId: string): number[] {
  ...
  const settled = new Set(
    all<{ number: number }>(
      // Either half is enough. `merge_state` is the durable fact; the stored lane is the
      // user dragging a card to Done, which is also a legitimate way to say "this is settled".
      "SELECT number FROM tickets WHERE project_id = ? AND (lane = 'done' OR merge_state = 'merged')",
      projectId,
    ).map((r) => r.number),
  )

  /*
   * A dependency on the same branch is satisfied by its work being **finished**, not merged.
   *
   * Once tickets in a chain share a worktree, the later one is literally built on top of the
   * earlier one's commits — the code is already there. Waiting for a merge would deadlock:
   * the branch cannot merge until every ticket on it is done, and the later ticket cannot
   * start until the branch merges.
   *
   * `ready_to_merge` is the marker, because that is the moment the earlier ticket's route
   * completed and its commits exist on the shared branch.
   */
  const readyOnBranch = new Set(
    all<{ number: number }>(
      'SELECT number FROM tickets WHERE project_id = ? AND ready_to_merge = 1',
      projectId,
    ).map((r) => r.number),
  )

  return deps.filter((n) => !settled.has(n) && !readyOnBranch.has(n))
}
