...
export async function syncWithBase(input: {
  ...
  // Same reconciliation the main-repo merge does: a generated file rewritten in the worktree
  // must not stop a catch-up that would otherwise be clean.
  await refreshIndex(input.worktree)
  ...
  try {
    ...
    return {
      ...
      behind,
      conflicts,
      reason: whatGitSaid(e),
    }
  }
}

/**
 * `git merge --squash`, with one recovery for the phantom that is not a real obstacle.
 *
 * `refreshIndex` runs before the pre-flight, but a container can rewrite a generated file in the
 * seconds between that and this — and then git refuses again for a file whose bytes never
 * changed. Rather than hand the user a refusal about work they did not do, the named files are
 * checked against HEAD: if every one is byte-identical, it is the same stale-mtime illusion, so
 * re-stat and try once more.
 *
 * Gated on content and capped at one attempt, so a genuine local change still stops the merge
 * and a real conflict cannot loop. Git names the files itself, one per line, which is what makes
 * this decidable rather than a guess.
 */
async function mergeSquashWithRetry(repo: string, branch: string): Promise<void> {
  try {
    await git(repo, ['merge', '--squash', branch])
    return
  } catch (e) {
    const named = overwriteVictims(e)
    if (named.length === 0) throw e
    if (!(await unchangedSinceHead(repo, named))) throw e
...
/**
 * The files git says would be overwritten, taken from its own message.
 *
 * Anything else — a conflict, a bad ref — returns empty and is rethrown untouched.
 */
function overwriteVictims(e: unknown): string[] {
  const out = e instanceof GitError ? e.gitOutput : ''
  if (!/would be overwritten by merge/i.test(out)) return []
  return out
    .split(SPLIT)
    .map((l) => l.trim())
    .filter((l) => l.length > 0)
    .filter((l) => !/^(error|hint|Please|Aborting|Updating|warning)/i.test(l))
}
...
> {
  ...
  /*
   * Make git and this function agree about what "modified" means, before either decides.
   *
   * They ask different questions. `blockingChanges` below compares **content**, which is the
   * honest answer to "would merging disturb something of yours". Git's merge safety check reads
   * the index's cached `(mtime, size)`, so a file rewritten with identical bytes — a generated
   * `payload-types.ts` that a container regenerates on every run — looks possibly-modified to it.
   *
   * The result was a merge this function had cleared and git then refused, on a file nobody had
   * edited. One re-stat reconciles them, and it cannot lose work: entries whose content really
   * differs stay dirty.
   */
  await refreshIndex(input.repo)
  ...
  try {
    ...
    return {
      ...
      kind: 'dirty',
      reason:
        `Could not switch to ${input.baseBranch}: ${whatGitSaid(e)}`,
      conflicts: [],
  ...
  try {
    await mergeSquashWithRetry(input.repo, input.branch)
  } catch (e) {
    let conflicts: string[] = []
    try {
  ...
  try {
    ...
    return {
      ...
      kind: 'error',
      reason:
        `Could not record the merge: ${whatGitSaid(e)}` + (restoreNote ? ` ${restoreNote}` : ''),
      conflicts: [],
    }
  }
