...
export function Messages({
  ...
  const busy =
    sending ||
    (pilot ? ['starting', 'thinking', 'working'].includes(pilot.status) : false)
  ...
  // A proposed route with a live ticket behind it. Anything else is history.
  const proposals = routes
    .filter((r) => r.status === 'proposed')
    .map((route) => ({ route, ticket: tickets.find((t) => t.id === route.ticketId) }))
    .filter((p): p is { route: TicketRoute; ticket: Ticket } => Boolean(p.ticket?.archivedAt === null))
    ...
    >
      ...
      <div ref={scrollRef} className="scroll-y" style={{ flex: 1, padding: '18px 20px 8px' }}>
        ...
        >
          ...
          {drafts.map((d) => (
            <DraftCard key={d.id} draft={d} projectId={project.id} />
          ))}

          {/*
            Nothing spawns until one of these is answered. The Pilot still decides the shape —
            this is where it shows you before acting on it.
          */}
          {proposals.map(({ route, ticket }) => (
            <PresentationCard
              key={route.id}
              route={route}
              ticket={ticket}
...
function LiveRow({ live, pilot }: { live: LiveText; pilot: Agent | null }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <Avatar
        ...
        {live.compacting ? (
          ...
          live.toolLine && <span className="meta">{live.toolLine.toLowerCase()}</span>
        )}
      </div>
      {live.text && (
        <Markdown
...
function DraftCard({ draft, projectId }: { draft: TicketDraft; projectId: string }) {
