 ...
 */
/** Same reasoning as `MessageRow`: it draws two names, and neither moves during a turn. */
const CommRow = memo(CommRowInner, (a, b) => {
  if (a.comm !== b.comm) return false
  const name = (list: Agent[], id: string | null): string | undefined =>
    list.find((g) => g.id === id)?.name
  return (
    name(a.agents, a.comm.fromAgentId) === name(b.agents, b.comm.fromAgentId) &&
    name(a.agents, a.comm.toAgentId) === name(b.agents, b.comm.toAgentId)
  )
})

function CommRowInner({ comm, agents }: { comm: Comm; agents: Agent[] }) {
...
/**
 * One row of the transcript, memoised on what it actually draws.
 *
 * The naive `memo` does nothing here: `agents` is a fresh array on every `agents:changed`, and
 * that fires constantly — a status line moving from "reading the project" to "ran a command"
 * is an agents event. So the comparator resolves the author on both sides and compares the six
 * display fields the row reads. None of them move during a turn; status and statusLine do, and
 * this row does not show either.
 *
 * The effect is the whole point: a token arriving in the live stream re-rendered all three
 * hundred rows and re-parsed every one of their markdown bodies, on the renderer's only
 * thread. That is what Windows was reporting as "not responding".
 */
const MessageRow = memo(MessageRowInner, (a, b) => {
  if (a.message !== b.message) return false
  const x = a.agents.find((g) => g.id === a.message.agentId) ?? null
  const y = b.agents.find((g) => g.id === b.message.agentId) ?? null
  if (x === y) return true
  if (!x || !y) return false
  return (
    x.id === y.id &&
    x.name === y.name &&
    x.avatarInitials === y.avatarInitials &&
    x.isPilot === y.isPilot &&
    x.model === y.model &&
    x.resolvedModel === y.resolvedModel
  )
})

function MessageRowInner({ message, agents }: { message: Message; agents: Agent[] }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '12px 13px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div className="selectable" style={{ fontSize: 14, fontWeight: 600, lineHeight: 1.45 }}>
          {q.question}
        </div>
        {q.context && (
          /*
           * The part you actually have to read, finally readable.
           *
           * This was raw text at 11px in muted grey — and an agent explaining three pricing
           * options writes lists, bold and headings, so what arrived was several hundred words
           * of markdown flattened into one long unbroken line of small type. The bullets were
           * there in the source. Nothing rendered them.
           *
           * Which made the choice below it undecidable: the buttons said "Variante B
           * (empfohlen)" and the only description of what B *was* had been mashed into a grey
           * blob with A and C. `Markdown` already exists and is already used for every other
           * body of agent text; this one was the exception, for no reason.
           *
           * Size and colour are the rest of it. This is primary content — it is the evidence
           * for a decision somebody is being asked to make right now — so it is set at reading
           * size in the ordinary ink, not as a footnote.
           */
          <div
            className="selectable"
            style={{
    ...
    <Blueprint style={{ border: '1px solid var(--line)', background: 'var(--surface)' }}>
      ...
      <div style={{ padding: '11px 13px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          ...
          <div style={{ flex: 1 }} />
          {/*
            Labelled, because unlabelled it is a riddle.

            This rendered as a bare phrase in the corner — "ein halber Tag" floating beside two
            buttons, with nothing saying what it measured. It is the Pilot's guess at how long
            the work takes, and it is a guess worth showing, but only once it says so.
          */}
          {draft.sizeNote && (
            <span className="meta" title="The Pilot's estimate of how long this takes">
              <span className="cap" style={{ color: 'var(--faint)', marginRight: 4 }}>
                est.
              </span>
              {draft.sizeNote}
            </span>
