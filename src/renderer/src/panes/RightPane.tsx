...
export function RightPane({ project, data }: { project: Project; data: ProjectData }) {
   ...
   *
   * This counted `LIVE_STATUSES`, which includes `waiting_on_you` — so a teammate stopped
   * dead on a question was reported as working, and a `blocked` one as idle. The strip's
   * whole job is saying what is happening, and it was wrong in both directions at once.
   * Holding a slot is the gate's question; this is a different one.
   */
  const working = data.agents.filter((a) => WORKING_STATUSES.includes(a.status))
  const needsYou = data.agents.filter((a) => NEEDS_YOU_STATUSES.includes(a.status))
  const idle = data.agents.filter(
    (a) => !WORKING_STATUSES.includes(a.status) && !NEEDS_YOU_STATUSES.includes(a.status),
  )
    ...
    >
      ...
      >
        ...
        {tokens > 0 && (
          <span
            className="meta tnum"
            // Said "cost against your rate limit" over a plain token count — the weighted
            // figure's wording on an unweighted number, contradicting the breakdown beneath it.
            title={`Tokens this project has put through a model.\n\n${tokenBreakdown(rawTotals)}`}
          >
      ...
      {data.quotaResetsAt && (
        ...
        >
          ...
          <span>
            {data.quotaStatus ? `${data.quotaStatus} — resets ` : 'Rate limited — resets '}
...
function AgentsList({
  ...
  working: Agent[]
  /** Alive and stopped — on a question, or out of budget. Your move, not theirs. */
  needsYou: Agent[]
  idle: Agent[]
  ...
  if (working.length === 0 && needsYou.length === 0 && idle.length === 0) {
    return (
      <Empty
        title="No agents"
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      {/* Between working and idle, because that is where it sits in reality: alive, and stopped. */}
      {needsYou.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <SectionRule label="Waiting for you" count={needsYou.length} />
          {needsYou.map((a) => (
            <AgentRow
              key={a.id}
              agent={a}
...
function AgentRow({
  ...
  return (
    ...
    >
      ...
      {/*
        A number where there can be no bar.

        Codex reports what it sent — every turn re-uploads the whole conversation, so that total
        is the context in use — but not the size of the window it fits in. The number is not in
        `model/list`, and `config/read` returns null for it because that key is a user override
        rather than the model's real size.

        So this is deliberately not a meter. A bar needs a denominator, the only available
        denominator would be one hardcoded per model, and a hardcoded window is the same trap as
        a hardcoded model list: it goes stale silently and reads as authoritative while it does.
        Half the fact, shown as half the fact.
      */}
      {agent.contextMax == null && agent.contextUsed != null && (
        <div
          className="meta tnum"
          title={
            `${agent.contextUsed.toLocaleString()} tokens of context on the last turn. ` +
            `Codex does not report the size of its context window, so there is no percentage ` +
            `to show — this is what it sent, not how full it is.`
          }
        ...
        >
          ...
          <span>{formatTokens(agent.contextUsed)}</span>
          <span style={{ color: 'var(--faint)' }}>sent</span>
        </div>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
        ...
        {/*
          The restart the status line has always promised. `markAllStalledOnBoot` says
          "stalled — restart it" and there was nothing to press: the worktree and session id
          both survived a close, and nothing in the app could spend them.
        */}
        {/*
          The escape hatch, asked for by name: *"why can't we just reset the senior dev?"*

          Every wedged teammate this week ended the same way — some stored state was wrong,
          nothing on screen could clear it, and the remedy was somebody finding the bad row. A
          reset does not need to know which piece went bad, because it drops all of them. It
          costs the conversation and never the branch, so it is safe to reach for first.

          Shown whenever an agent is not busy: needing it is precisely the situation where the
          app's own account of what is wrong cannot be trusted.
        */}
        {agent.status !== 'working' && agent.status !== 'thinking' && !agent.isPilot && (
          <button
            onClick={() => {
              void window.vibepilot.agents.reset(agent.id)
            }}
            title="Forgets its saved conversation and starts it clean. Its branch, worktree and commits are untouched."
            style={{
              border: 'none',
              background: 'transparent',
              color: 'var(--faint)',
        ...
        {agent.status === 'stalled' && (
          <button
            onClick={() => {
              void window.vibepilot.agents.restart(agent.id)
            }}
            title="Resumes where it left off, in the same worktree, with its context intact"
            style={{
              display: 'inline-flex',
              alignItems: 'center',
              gap: 3,
...
function BranchesTab({
  ...
  // Same unguarded shape as the merge button had, and the same stuck-forever failure.
  const push = async (): Promise<void> => {
    ...
    try {
      ...
      setNote((e as Error)?.message ?? 'The push failed, and did not say why.')
    } finally {
      setBusy(false)
      load()
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {/*
        What have my agents got — and only what they still have.

        This listed every ticket branch ever made, labelled from `ahead`, which counts commits.
        vibePilot merges by squashing, so a branch that landed perfectly keeps its own commits
        and keeps counting them: ten branches, five of them entirely dead, and no way to tell
        which from the panel. `landed` asks about content instead, and the dead ones fold away.
      */}
      {ov && ov.ticketBranches.some((b) => !b.landed) && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
          <SectionRule
            label="Ticket branches"
            count={ov.ticketBranches.filter((b) => !b.landed).length}
          />
          {ov.ticketBranches
            .filter((b) => !b.landed)
            .map((b) => (
              <Row
                key={b.name}
                name={b.name}
                right={<span className="meta tnum">{`↑${b.ahead}`}</span>}
              />
            ))}
        </div>
      )}
      ...
      {ov && ov.ticketBranches.some((b) => b.landed) && (
        <LandedBranches
          branches={ov.ticketBranches.filter((b) => b.landed).map((b) => b.name)}
...
/**
 * Branches whose work is already on the base, out of the way but not gone.
 *
 * Nothing ever retired one, so the panel grew: ten branches on a real project, five of them
 * with nothing left on them, and the label — computed from a commit count that a squash merge
 * leaves untouched — insisting they were unmerged work. Reading what was actually outstanding
 * meant checking each one by hand in a terminal.
 *
 * Collapsed by default and deleted only on request. Deleting a branch is the one thing in this
 * panel git cannot undo, so however confident the landed-check is, it does not get to act on
 * its own: the app can stop showing them, and the user decides whether they go.
 */
function LandedBranches({
  branches,
  base,
  projectId,
  onDone,
  ...
  const remove = async (): Promise<void> => {
    ...
    setFailed(null)
    try {
      const r = await window.vibepilot.git.deleteLandedBranches(projectId)
      if (!r.ok) setFailed(r.reason ?? 'They could not be deleted.')
      else if (r.kept?.length) {
        setFailed(
          `${r.kept.length} kept — still checked out somewhere, or no longer safe to remove.`,
        )
      }
      onDone()
    } catch (e) {
      setFailed(e instanceof Error ? e.message : 'They could not be deleted.')
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      <SectionRule label="Landed" count={branches.length} />
      <span className="meta" style={{ lineHeight: 1.45, whiteSpace: 'normal' }}>
        Every line of these is already on <code>{base}</code>. Nothing is waiting on them.
      </span>
      ...
      <div style={{ display: 'flex', gap: 7, alignItems: 'center', flexWrap: 'wrap' }}>
        <Button height={24} onClick={() => setOpen((v) => !v)}>
          {open ? 'Hide' : `Show ${branches.length}`}
        </Button>
        <Button height={24} disabled={busy} onClick={() => void remove()}>
...
function ReadyList({
  ...
  projectId: string
  baseBranch: string
  /** Changes to files git already tracks. These genuinely obstruct a merge. */
  unsaved: string[]
  /** Files git has never seen. Mentioned, not treated as an obstacle. */
  untracked: string[]
  onDone: () => void
}) {
  if (tickets.length === 0) {
  ...
  /*
   * One card per branch, not per ticket.
   *
   * Tickets built on top of each other share a branch and land in a single squash-merge — the
   * engine has worked that way since branch grouping, and refuses to merge such a branch until
   * every ticket on it is finished. The list here still drew a card and a button per ticket, so
   * two tickets on one branch showed two identical *Merge into main* buttons above two
   * identical branch names. Pressing either one lands both; pressing the second does nothing.
   * There was no way to tell that from looking at it.
   */
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {[...groups.entries()].map(([key, group]) => {
        // The lowest-numbered ticket carries the merge: `mergeTicket` lands the whole branch
        // whichever member it is handed, and the lowest is the one the branch is named after.
        const lead = [...group].sort((a, b) => a.number - b.number)[0]!
        const who = agents.find((a) => a.id === lead.assigneeAgentId)
        return (
          <div
            key={key}
            style={{
          ...
          >
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 7 }}>
              <span className="meta tnum">
                {group.map((t) => `#${t.number}`).join(' + ')}
              </span>
              {group.some((t) => t.mergeState === 'conflict') && <Tag tone="danger">conflict</Tag>}
              <div style={{ flex: 1 }} />
              {who && <span className="meta">{who.name}</span>}
            </div>
            {group.map((t) => (
              <div key={t.id} style={{ fontSize: 12.5, fontWeight: 500, lineHeight: 1.45 }}>
                {group.length > 1 && (
                  <span className="meta tnum" style={{ marginRight: 5 }}>
                    #{t.number}
                  </span>
                )}
                {t.title}
            ...
            {lead.branch && <span className="meta mono ellip">{lead.branch}</span>}
            {group.length > 1 && (
              <span className="meta" style={{ lineHeight: 1.5, whiteSpace: 'normal' }}>
                Built on top of each other, so one merge lands all {group.length}.
              </span>
            )}
            <MergeButton
              projectId={projectId}
...
function MergeButton({
  ...
  ticket: Ticket
  baseBranch: string
  /** Changes to files git already tracks. Empty means this is one uneventful press. */
  unsaved: string[]
  /** Files git has never seen. Mentioned, never treated as an obstacle. */
  untracked: string[]
  onDone: () => void
}) {
  const [busy, setBusy] = useState(false)
  ...
  /*
   * `finally`, and it is the whole fix.
   *
   * The await was unguarded, so anything that made the IPC *throw* rather than return
   * `{ ok: false }` — a git command failing in a way `squashMerge` did not model, a branch
   * merged outside the app, the handler itself erroring — skipped `setBusy(false)` entirely.
   * The button then read "Merging…" for ever, with no error beside it, while nothing was
   * happening. A stuck spinner is the same lie as a stale board: it reports an action still in
   * progress that ended some time ago.
   */
  const merge = async (setAside = false) => {
    ...
    try {
      ...
      setError((e as Error)?.message ?? 'The merge failed, and did not say why.')
    } finally {
      setBusy(false)
      onDone()
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      {/*
        Untracked files, mentioned rather than treated as an obstacle.

        These used to count as "your unsaved work", so a merge announced itself as blocked by
        four `.claude/skills/` folders the user had never edited and did not think of as
        theirs. Git reports them; that does not make them a conflict. They collide only if the
        incoming work adds the same path, and git says so at the time if it does.
      */}
      {untracked.length > 0 && unsaved.length === 0 && (
        <div className="meta" style={{ color: 'var(--faint)', lineHeight: 1.5 }}>
          {untracked.length} file{untracked.length === 1 ? '' : 's'} here{' '}
          {untracked.length === 1 ? 'is' : 'are'} not tracked by git ({untracked[0]}
          {untracked.length > 1 ? `, and ${untracked.length - 1} more` : ''}). The merge leaves
          them alone. Add them to <code>.gitignore</code> to stop seeing this.
        </div>
      )}
    </div>
  )
