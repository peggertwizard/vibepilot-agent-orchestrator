...
export function Team({
  ...
  /** The repo scan is a real Claude process and takes a while — say so while it runs. */
    ...
    <div className="scroll-y" style={{ flex: 1, padding: '20px 20px 28px' }}>
      <div style={{ maxWidth: 820, display: 'flex', flexDirection: 'column', gap: 18 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
          ...
          {/*
            Ask the app to look at the repo and propose a team.

            This scan already existed and was offered exactly once, on an empty project, as a
            card in the message stream — decline it and there was no way back to it, ever. It
            is the same read-only pass (it can Read, Glob and Grep, and nothing else), and it
            is the obvious thing to want when you are staring at a Team tab wondering who to
            hire.
          */}
          <Button
            height={28}
            disabled={scanning}
            title="Reads the project and suggests teammates. It cannot change any files."
            onClick={() => {
              setScanning(true)
              void window.vibepilot.projects.bootstrap(project.id).finally(() => setScanning(false))
            }}
          >
            {scanning ? 'Reading the repo…' : 'Suggest from this repo'}
          </Button>
      ...
      {editing && (
        <TeammateEditor
          /*
           * Keyed on who is being edited. The editor seeds its fields from `agent` in
           * `useState` initialisers, which never re-run — so opening one teammate, closing,
           * and opening another showed the first one's name, model and instructions over the
           * second one's row.
           */
...
function TeammateEditor({
  ...
  /*
   * The Codex models this machine has, asked of the CLI when the editor opens.
   *
   * Not a constant, and deliberately never one. Every OpenAI model id vibePilot could hardcode
   * is one that gets renamed or retired, and the failure would be silent — a picker still
   * offering `gpt-5.4` long after it stopped existing. The installed binary knows; ask it.
   */
  const [codexModels, setCodexModels] = useState<CodexModel[]>([])
  useEffect(() => {
    let live = true
    void window.vibepilot.codex
      .models()
      .then((m) => live && setCodexModels(m))
      .catch(() => undefined)
    return () => {
      live = false
    }
  ...
  const codexChosen = codexModels.find((m) => CODEX_PREFIX + m.id === model) ?? null
  /*
        ...
        >
          ...
          <Field label="Model" hint="Chosen per teammate. There is no default.">
            ...
            */}
            ...
            {/*
              Which Codex, chosen per teammate.

              Codex has no aliases to stand in for "latest", so the only honest options are the
              ones the installed CLI reports — id, name and description all come straight from
              it. Update Codex and this list updates; nothing here can name a model that has
              stopped existing.
            */}
            {(provider === 'codex' || codexModels.length > 0) && model === 'codex' && (
              <div style={{ marginTop: 8 }}>
                ...
                {codexModels.length === 0 ? (
                  <div className="meta" style={{ color: 'var(--faint)', lineHeight: 1.5 }}>
                    Could not read the model list from your Codex install, so it will use
                    whatever Codex is configured to use. Check the Codex row in Doctor.
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {codexModels.map((m) => (
                      <Pill
                        ...
                        on={false}
                        onClick={() => {
                          setModel(CODEX_PREFIX + m.id)
                          // Its own default beats ours: the levels differ per model, and this
                          // one is what Codex would have picked anyway.
                          if (m.defaultEffort && isValidEffort(vpEffort(m.defaultEffort))) {
                            setEffort(vpEffort(m.defaultEffort) as EffortLevel)
                          }
                        }}
                      >
                        {m.label}
            ...
            {codexChosen && (
              <div className="meta" style={{ marginTop: 5, lineHeight: 1.5 }}>
                <strong>{codexChosen.label}</strong>
                {codexChosen.description ? ` — ${codexChosen.description}` : ''}
                <button
            ...
            */}
            <div style={{ marginTop: 8, display: provider === 'codex' ? 'none' : undefined }}>
              <div className="cap" style={{ marginBottom: 4 }}>
          ...
          >
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
              {EFFORT_OPTIONS.map((o, i) => {
                 ...
                 *
                 * For Codex the answer is per model and comes from the CLI: `gpt-5.4-mini`
                 * accepts neither `max` nor `ultra`, while `gpt-5.6-sol` takes both. Offering a
                 * level the model rejects turns a click into a failed launch minutes later, so
                 * the list is narrowed to what was actually reported for the chosen model.
                 */
                const dead = codexChosen
                  ? !codexChosen.efforts.map(vpEffort).includes(o.id)
                  : !modelTakesEffort
                return (
                  <Fragment key={o.id}>
                    {/*
                    ...
                    */}
                    ...
                    <button
                      ...
                      title={
                        dead
                          ? codexChosen
                            ? `${codexChosen.label} does not offer ${o.label.toLowerCase()}`
                            : `${modelLabel(model)} has no thinking levels`
                          : o.note
                      }
                      style={{
