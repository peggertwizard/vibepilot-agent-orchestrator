...
export function Memory({ project }: { project: Project }) {
   ...
   *
   * Typing produces a request per keystroke and they do not come back in order — a slow
   * three-character search resolving after a fast five-character one left the list showing
   * results for a query the box no longer contained. Only the newest may write.
   */
  const seq = useRef(0)
  const load = useCallback(
    (q: string) => {
      const mine = ++seq.current
      void window.vibepilot.memory.search(project.id, q).then((r) => {
        if (mine === seq.current) setEntries(r)
      })
    },
    [project.id],
  )

  /** Read inside the bus handler so a refresh keeps the current query without re-subscribing. */
  ...
  useEffect(() => {
    ...
    return off
    /*
     * `query` is deliberately not a dependency. It was, which tore down and rebuilt the bus
     * subscription on every keystroke and fired a racing `load('')` against the `load(v)` the
     * input had just issued — so the box could say "auth" while the list showed everything.
     */
  }, [project.id, load])
