...
export function CentrePane({
  ...
  const needsYou: NeedsYouItem[] = buildNeedsYouItems({
    ...
    /*
     * Deciding on a draft without opening it. The IPC lives here rather than in the popover so
     * the list builder stays a pure function with its own tests.
     */
    act: {
      acceptDraft: async (id) => {
        await window.vibepilot.drafts.accept(project.id, id)
      },
      rejectDraft: async (id) => {
        await window.vibepilot.drafts.reject(project.id, id)
      },
    },
