...
export function CentrePane({
  ...
  const readyToMerge = live.filter((t) => t.readyToMerge).length

  /*
   * Work that says it is running and is not.
   *
   * A step sits `active` with an assignee who has no process and is not queued. Until the
   * launch queue existed this happened whenever two tickets were routed to the same teammate:
   * the second launch was dropped and the card waited for ever beside an idle agent. It should
   * never happen now — but if it does, it must be visible rather than silent.
   */
  const stuck = data.routes.filter((r) => {
    if (r.status !== 'accepted') return false
    const step = r.steps.find((s) => s.status === 'active' || s.status === 'rework')
    if (!step?.assigneeAgentId) return false
    const who = data.agents.find((a) => a.id === step.assigneeAgentId)
    return !!who && !LIVE_STATUSES.includes(who.status) && who.status !== 'queued'
  }).length

  const waiting =
    data.questions.length + data.drafts.length + proposedRoutes + readyToMerge + data.hires.length
