...
export function Settings({
  ...
  /** The project that was open when Settings was opened — the starting selection. */
  project: Project
  /** All of them, because settings for a project you are not looking at were unreachable. */
  projects: Project[]
  ...
  /*
   * Which project this is editing.
   *
   * It could only ever edit the active one. Both entry points — the gear on the sidebar row
   * and the gear in the footer — call `onOpenSettings()` with no argument, so a second
   * project's base branch, checks, spend ceiling and auto-merge setting were simply not
   * reachable without switching the whole app over to it first.
   */
  const [selectedId, setSelectedId] = useState(project.id)
  const selected = projects.find((p) => p.id === selectedId) ?? project

  /** The roster belongs to a project, so a different selection needs a different list. */
  const [selectedAgents, setSelectedAgents] = useState<Agent[]>(agents)
  useEffect(() => {
    if (selectedId === project.id) {
      setSelectedAgents(agents)
      return
    }
    void window.vibepilot.agents.list(selectedId).then(setSelectedAgents)
  }, [selectedId, project.id, agents])
  /**
  ...
  const asDraft = (p: Project) => ({
    ...
    autoArchiveDays: p.autoArchiveDays,
  })
  const teammateCount = Math.max(1, selectedAgents.filter((a) => !a.isPilot).length)
  const [draft, setDraft] = useState(() => asDraft(project))
  /*
   * Reseeded when you switch which project you are editing — and only then.
   *
   * This depended on `project`, the object. `refreshProjects` rebuilds the whole list on every
   * `projects:changed`, producing a new object each time, and four controls on this very
   * screen (auto-start, auto-merge, pause, trust) write immediately and emit that event. So
   * flipping any one of them silently wiped whatever you had typed into Name, Base branch or
   * the check commands. Keying on the id makes background refreshes invisible here.
   */
  useEffect(() => setDraft(asDraft(selected)), [selectedId])
  ...
  const dirty = JSON.stringify(draft) !== JSON.stringify(asDraft(selected))

  const save = async (): Promise<void> => {
    await window.vibepilot.projects.update(selected.id, {
      name: draft.name.trim() || selected.name,
      defaultBaseBranch: draft.defaultBaseBranch.trim() || selected.defaultBaseBranch,
    ...
    >
      ...
      >
        ...
        */}
        <Tabs
          tabs={[
            { id: 'project' as const, label: selected.name },
        ...
        {tab === 'project' && projects.length > 1 && (
          /*
            Which project you are editing.

            Both ways into this screen pass no argument at all, so it always showed the active
            project and there was no way to reach any other one's settings — you had to switch
            the whole app over first, which is a strange thing to have to do to change a base
            branch.
          */
          <div
            style={{
          ...
          >
            <span className="cap">Settings for</span>
            {projects
              ...
              .map((p) => {
                ...
                return (
                  <button
                    ...
                    aria-pressed={on}
                    onClick={() => {
                      // Switching with edits pending would throw them away silently, which is
                      // the exact failure the confirm-on-close dialog exists to prevent.
          ...
          <>
              ...
              */}
              <div>
                <Button
                  height={24}
                  onClick={() => {
                    void window.vibepilot.projects.detectChecks(selected.id).then((p) => {
            ...
            */}
            <Section title="Where finished work goes">
              <EnvironmentsEditor projectId={selected.id} />
            </Section>
            ...
            <Section title="Starting new work">
                ...
                {AUTO_START_OPTIONS.map((o) => (
                  <label
                    ...
                    style={{
                      ...
                      padding: '7px 9px',
                      border: `1px solid ${
                        selected.autoStart === o.id ? 'var(--accent)' : 'var(--line)'
                      }`,
                      background:
                        selected.autoStart === o.id ? 'var(--accent-soft)' : 'transparent',
                      cursor: 'pointer',
                    }}
                  >
                    <input
                      type="radio"
                      name="autoStart"
                      checked={selected.autoStart === o.id}
                      onChange={() =>
                        void window.vibepilot.projects.update(selected.id, { autoStart: o.id })
                      }
                      style={{ marginTop: 2 }}
                    />
                  ...
                  {AUTO_MERGE_OPTIONS.map((o) => (
                    <label
                      ...
                      style={{
                        ...
                        padding: '7px 9px',
                        border: `1px solid ${
                          selected.autoMerge === o.id ? 'var(--accent)' : 'var(--line)'
                        }`,
                        background:
                          selected.autoMerge === o.id ? 'var(--accent-soft)' : 'transparent',
                        cursor: 'pointer',
                      }}
                    >
                      <input
                        type="radio"
                        name="autoMerge"
                        checked={selected.autoMerge === o.id}
                        onChange={() =>
                          void window.vibepilot.projects.update(selected.id, { autoMerge: o.id })
                        }
                        style={{ marginTop: 2 }}
                      />
                      <span style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <Button
                  height={26}
                  kind={selected.launchPaused ? 'primary' : 'ghost'}
                  onClick={() =>
                    void window.vibepilot.projects.update(project.id, {
                      launchPaused: !selected.launchPaused,
                    })
                  }
                >
                  {selected.launchPaused ? 'Resume starting work' : 'Pause starting work'}
                </Button>
                <span className="meta">
                  {selected.launchPaused
            ...
            <Section title="The Pilot, on this project">
              ...
              */}
              ...
              {/*
                The Done column only ever grew, and the archive was one ticket at a time from a
                menu behind an ellipsis. On by default at three days: long enough to see what
                landed over a weekend, short enough that the board stays a board.
              */}
              {/*
                Moved here from the vibePilot tab, where it never belonged: it names a project
                in its own prose and calls `resetUsage(project)`. The tab it sat on is supposed
                to be the things that are true everywhere.
              */}
              <Field
                label="Token counters"
                hint="Counters add up for as long as a teammate exists. Resetting starts them from zero for this project — tickets, messages and history are untouched."
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <Button
                    height={26}
                    onClick={() => {
                      void window.vibepilot.agents.resetUsage(selected.id).then((n) => setResetNote(n))
                    }}
                  >
                    Reset counters
                  </Button>
              ...
              <Field
                label="Archive finished tickets after"
                hint="Days a ticket stays on the board once it is done. It is moved to the archive, never deleted. 0 keeps everything."
              >
                <Input
                  value={String(draft.autoArchiveDays)}
                  onChange={(v) =>
                    setDraft((d) => ({
                      ...d,
                      autoArchiveDays: Math.max(0, Math.min(365, Math.round(Number(v) || 0))),
                    }))
                  }
                  height={28}
                  placeholder="3"
        ...
        <Section title="This folder's own Claude settings">
          ...
          */}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 8 }}>
            <Button
              height={26}
              kind={selected.settingsTrusted ? 'ghost' : 'primary'}
              onClick={() =>
                void window.vibepilot.projects.update(project.id, {
                  settingsTrusted: !selected.settingsTrusted,
                })
              }
            >
              {selected.settingsTrusted ? 'Stop trusting this folder' : 'Trust this folder'}
            </Button>
            <span className="meta">
              {selected.settingsTrusted
        ...
        <Section title="Files vibePilot keeps in your repo">
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <Button
              height={26}
              onClick={() => void window.vibepilot.system.revealInExplorer(`${selected.path}/.vibepilot`)}
            >
              Open .vibepilot
            </Button>
            <Button height={26} onClick={() => void window.vibepilot.memory.openFolder(selected.id)}>
              Open memory
            </Button>
          </div>
        </Section>
        ...
        {tab === 'app' && (
          <>
        {/* Said once, plainly. The two tabs look alike and the difference matters. */}
        <p className="meta" style={{ color: 'var(--faint)', lineHeight: 1.6 }}>
          These apply everywhere, on every project. Anything belonging to one project is on the
          other tab.
        </p>
