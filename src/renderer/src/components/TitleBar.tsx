...
function UpdateChip() {
  ...
  const shown = state.phase === 'available' || state.phase === 'downloading' || state.phase === 'ready'
  if (!shown) return null

  /*
   * Three states, three different verbs — and only two of them are clickable. Nothing is
   * downloaded until the first click, and nothing is installed until the second.
   */
  const label =
  ...
  const act = (): void => {
    if (state.phase === 'available') void window.vibepilot.update.download()
    // Closing is the install: the app drains its agents, saves, installs, and reopens itself.
    if (state.phase === 'ready') void window.vibepilot.window.close()
  }

  const ready = state.phase !== 'downloading'
  ...
  return (
    <button
      ...
      title={
        state.phase === 'available'
          ? `Version ${state.version} is available. Nothing has been downloaded yet — clicking starts it.`
          : state.phase === 'ready'
            ...
            : 'Downloading. Nothing is interrupted, and nothing installs until you close the app.'
      }
      style={{
        display: 'inline-flex',
        alignItems: 'center',
