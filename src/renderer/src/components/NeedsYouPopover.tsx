...
export interface NeedsYouItem {
  ...
  /**
   * Yes and no, on the row itself.
   *
   * A proposed ticket has exactly two answers and both were behind a disclosure: you had to
   * press "more", read a card you had already read in the message stream, and only then find
   * the buttons. For the commonest item in the list that is two clicks and a wall of text to
   * say yes.
   *
   * Deliberately only for items where the choice really is binary. A question with five
   * suggested answers still opens.
   */
  act?: { accept: () => Promise<void>; reject: () => Promise<void>; acceptLabel: string }
}

const KIND_LABEL: Record<NeedsYouKind, string> = {
  branch: 'your project folder',
  question: 'a question',
  gate: 'your sign-off',
  split: 'a breakdown',
...
const URGENT: NeedsYouKind[] = ['branch', 'question', 'gate', 'stuck']
...
export function buildNeedsYouItems(input: {
  ...
  hires: HireProposal[]
  agents: Agent[]
  /** What git says the project folder is on, when that is not where merges land. */
  divergedFrom?: { here: string; base: string; ahead: number } | null
  /** Deciding, without opening anything. Supplied by the pane that owns the IPC. */
  act: {
    acceptDraft: (draftId: string) => Promise<void>
    rejectDraft: (draftId: string) => Promise<void>
  }
  render: {
    branch: (d: { here: string; base: string; ahead: number }) => React.ReactNode
  ...
  /*
   * First, above even a blocking question — because while this is true, everything else you
   * might do about the others is being reported against the wrong copy of the project.
   *
   * The whole model rests on this one fact: teammates branch from the project folder, merges
   * land in it, and anything watching it — a dev server, a container — renders what it holds.
   * Let it drift onto a feature branch and all three silently mean something else. Merged work
   * stops appearing, and every obvious explanation for that is wrong, so the search goes
   * anywhere except here.
   */
  if (input.divergedFrom) {
    out.push({
      id: 'branch-drift',
      kind: 'branch',
      title: `Your project folder is on ${input.divergedFrom.here}, not ${input.divergedFrom.base}`,
      summary:
        `Merges land in ${input.divergedFrom.base}, so finished work is not what this folder — ` +
        `or anything serving it — is showing you right now.` +
        (input.divergedFrom.ahead > 0
          ? ` And ${input.divergedFrom.ahead} ${
              input.divergedFrom.ahead === 1 ? 'commit is' : 'commits are'
            } only here.`
          : ''),
      detail: input.render.branch(input.divergedFrom),
    })
  }
  ...
  for (const d of input.drafts) {
    out.push({
       ...
       * Yes and no, on the row itself.
       *
       * A proposed ticket has exactly two answers and both were behind "more": one press to
       * reveal a card already visible in the message stream, and a second to answer it.
       * Opening still works and is still where you go to edit the title before accepting.
       */
      act: {
        accept: () => input.act.acceptDraft(d.id),
        reject: () => input.act.rejectDraft(d.id),
        acceptLabel: 'Create ticket',
      },
    })
  }
   ...
   *
   * Driven by `ticket.stuck`, which `derivePlacement` already computes centrally — not by
   * agent status, which is what this did first and which disagreed with the header. The header
   * said "1 ticket is stuck — nobody is actually working on it" while every teammate read
   * `idle`, so the button counted a thing its own list could not show. Two derivations of one
   * fact, which is the mistake the whole board rewrite existed to stop making.
   *
   * `laneBecause` carries the real reason: assigned-but-not-running, or nobody assigned at all.
   */
  for (const t of live) {
    if (!t.stuck) continue
    const who = input.agents.find((a) => a.id === t.assigneeAgentId && !a.isPilot) ?? null
    out.push({
      id: `stuck-${t.id}`,
      kind: 'stuck',
      title: who ? `${who.name} stopped on #${t.number}` : `#${t.number} is not moving`,
      summary: t.laneBecause || who?.statusLine || 'Nothing is running it.',
      // Only offer a restart when there is somebody to restart. A step with nobody on it is a
      // different decision, and a Restart button that cannot work is worse than no button.
      detail: who ? input.render.stuck(who) : undefined,
      go: { tab: 'board', ticketId: t.id, label: 'Open ticket' },
    })
  }
...
/**
 * Accept or discard, without opening anything.
 *
 * The tick is the primary act and the cross sits beside it, both labelled for a screen reader
 * because a glyph alone is not a control. Disabled together while either runs, so a double
 * press cannot create the ticket and then reject the draft that made it.
 */
function RowActions({
  act,
}: {
  ...
  const run = (fn: () => Promise<void>) => (e: React.MouseEvent) => {
    // The row itself toggles the disclosure; these must not do both.
    e.stopPropagation()
    setBusy(true)
  ...
  return (
    <>
      <button
        onClick={run(act.accept)}
        disabled={busy}
        title={act.acceptLabel}
        aria-label={act.acceptLabel}
        style={{ ...base, borderColor: 'var(--accent)', color: 'var(--accent)' }}
      >
        ✓
      </button>
      <button
        onClick={run(act.reject)}
        disabled={busy}
        title="Discard"
        aria-label="Discard"
        style={{ ...base, color: 'var(--faint)' }}
      >
        ✕
      </button>
    </>
