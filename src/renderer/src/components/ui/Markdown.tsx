 ...
 *    headings, bullet and numbered lists, blockquotes, links, horizontal rules, and tables.
 *    Tables were left out at first as "rare in chat" — which turned out to be wrong in the
 *    most visible way possible. Models reach for one whenever they compare things, and an
 *    unparsed table is worse than no table: a wall of pipes and dashes where a comparison
 *    should be. Footnotes stay out; those really are rare.
 *
 ...
 *
 * Requires at least one pipe and nothing but pipes, dashes, colons and spaces — which is what
 * separates a real table from a line of prose that happens to contain a dash and a pipe.
 */
const SEPARATOR = /^\s*\|?[\s:|-]*-[\s:|-]*\|[\s:|-]*$/

/** Split a row on unescaped pipes, dropping the empty cells the outer pipes create. */
...
function blocks(src: string): ReactNode[] {
  ...
  while (i < lines.length) {
    ...
    /*
     * A table, in the one shape models actually write: a header row, a separator, then rows.
     *
     * Detection requires the separator on the *next* line, not just a pipe on this one —
     * otherwise any sentence containing "a | b" would swallow the paragraph after it.
     */
    if (line.includes('|') && i + 1 < lines.length && SEPARATOR.test(lines[i + 1]!)) {
      ...
      push(
        // Its own scroll box. A wide comparison must not make the whole message column
        // scroll sideways — that moves the text you are reading, to show you a column.
        <div key={key++} style={{ margin: '0 0 0.65em', overflowX: 'auto' }}>
          <table
            style={{
