...
describe('actually merging', () => {
  ...
  /*
   * Only when the two genuinely overlap.
   *
   * This used to dirty `base.js`, which no ticket branch ever touches — and that case is now
   * allowed through, because the merge cannot harm it and git leaves it exactly where it is.
   * Refusing on it is what blocked every merge in a project carrying a file the dev server
   * rewrites on every run.
   */
  it('stops when its work would land on top of yours, rather than moving yours', async () => {
    const t = builtTicket()
    /*
     * A file that exists on both sides. `builtTicket` only *adds* a file, and an added path is
     * untracked here — a case git guards itself, and which arrives as a plain error carrying
     * git's own wording rather than as this refusal.
     */
    gitq(['checkout', '-q', t.branch!])
    writeFileSync(join(repo, 'base.js'), 'the agent rewrote this\n')
    gitq(['add', '-A'])
    gitq(['commit', '-qm', 'touch base'])
    gitq(['checkout', '-q', 'main'])

    const mine = 'base.js'
    writeFileSync(join(repo, mine), 'mine, not saved\n')

    const res = await mergeTicket(t.id, { auto: true })
    expect(res.ok).toBe(false)
    if (!res.ok) {
      expect(res.kind).toBe('dirty')
      expect(res.reason, 'names the file').toContain(mine)
    }

    // Untouched — the whole point of stopping.
    expect(gitq(['status', '--porcelain'])).toContain(mine)
    gitq(['reset', '-q', '--hard'])
    gitq(['clean', '-qfd'])
  })
