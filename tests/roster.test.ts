...
describe('role definitions', () => {
  ...
  it('still says plainly what a Codex teammate cannot do', () => {
    ...
    expect(CODEX_LIMITATIONS.join(' ')).toMatch(/no context-window reporting/)
  })

  /**
   * And nothing it *can* do.
   *
   * "no sub-agents, so it cannot run its own checks" was asserted here for as long as it was
   * false. Codex's `ultra` effort is described by the CLI itself as maximum reasoning with
   * automatic task delegation, and `codex doctor` reports real `subagent:thread_spawn` sessions
   * in the user's own history. Warning somebody off a capability that exists is the same class
   * of mistake as promising one that does not — this pins the correction so it cannot drift
   * back.
   */
  it('no longer claims Codex has no sub-agents', () => {
    expect(CODEX_LIMITATIONS.join(' ')).not.toMatch(/sub-agent/)
  })
})
