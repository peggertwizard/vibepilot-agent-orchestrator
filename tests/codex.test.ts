...
/**
 * Codex counts for the whole thread, not for the turn.
 *
 * Measured against codex-cli 0.145.0 on a trivial two-turn thread: input went 19,051 → 39,686
 * and output went 5 → 10. `turn.completed.usage` is a running total. Everything downstream
 * *adds* what it is handed, so each turn re-added everything before it and the totals grew
 * quadratically — a single text ticket showed **2.65M tokens** and a context reading of
 * **1.39M**. Not large numbers; wrong ones.
 *
 * The adapter subtracts, which fixes both at once: the delta is what the turn actually cost,
 * and — because Codex re-uploads the whole conversation every turn — it is also exactly how
 * much context was in play.
 */
describe('turning a running total back into a turn', () => {
  const cost = (input: number, output: number) =>
    run([{ type: 'turn.completed', usage: { input_tokens: input, output_tokens: output } }])[0]!

  it('the translator still reports exactly what Codex said', () => {
    const e = cost(39_686, 10)
    if (e.type !== 'agent:cost') throw new Error('expected a cost event')
    // Raw, cumulative — the subtraction belongs to the adapter, which survives across turns.
    expect(e.inputTokens).toBe(39_686)
    expect(e.outputTokens).toBe(10)
  })
})
