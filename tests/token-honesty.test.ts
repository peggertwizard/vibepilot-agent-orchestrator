...
describe('counting tokens', () => {
  ...
  it('counts each distinct token once, and cache reads not at all', () => {
    ...
    // 127 sent + 47,153 written back. Neither cache figure is work anyone did.
    expect(totalTokens(sam)).toBe(47_280)
  ...
  it('does not let a long conversation inflate a short turn', () => {
    const short = { tokensIn: 10, tokensOut: 90, tokensCacheWrite: 4000 }
...
describe('a message costs what it costs', () => {
  ...
  it('does not read 740k for one reply', () => {
    ...
    // 14 sent + 2,958 written back — the scale Claude's own display shows for a turn.
    expect(totalTokens(realTurn)).toBe(2_972)
    expect(totalTokens(realTurn)).toBeLessThan(raw / 200)
  })
