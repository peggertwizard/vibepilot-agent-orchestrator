...
import { TOKEN_WEIGHTS, totalTokens } from '../src/shared/types'
...
describe('ticket spend', () => {
  ...
  it('keeps the four raw token fields, so the weighting happens in one place', () => {
    ...
    /*
     * The weighted figure is what gets displayed. A raw sum counts the same cached
     * conversation once per API round-trip, which is where the 41× inflation came from —
     * 225k of distinct content reading as 9.27M.
     */
    const weighted = totalTokens({
      tokensIn: s.tokensIn,
    ...
    expect(weighted).toBe(
      Math.round(
        s.tokensIn * TOKEN_WEIGHTS.in +
          s.tokensOut * TOKEN_WEIGHTS.out +
          s.tokensCacheRead * TOKEN_WEIGHTS.cacheRead +
          s.tokensCacheWrite * TOKEN_WEIGHTS.cacheWrite,
      ),
    )
  })
