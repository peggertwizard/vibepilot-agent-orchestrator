...
/**
 * Tables, which the renderer used to leave as a wall of pipes.
 *
 * They were left out as "rare in chat", which was wrong in the most visible way: a model
 * reaches for a table whenever it compares things, and an unparsed one is strictly worse than
 * no table at all — the comparison is exactly the thing that becomes unreadable.
 */
describe('tables', () => {
  const table = [
    '| Component | Uses | Verdict |',
    '|---|---:|---|',
    '| `CLAUDE.md` | — | keep — already lean |',
    '| skill | 9 | keep |',
  ].join('\n')

  it('renders a real table rather than the pipes', () => {
    const html = renderToStaticMarkup(<Markdown text={table} />)
    expect(html).toContain('<table')
    expect(html).toContain('<th')
    expect(html).toContain('Component')
    expect(html).toContain('keep — already lean')
    expect(html, 'no raw separator row').not.toContain('|---|')
  })

  it('keeps inline formatting inside cells', () => {
    const html = renderToStaticMarkup(<Markdown text={table} />)
    // `CLAUDE.md` is code in the source and must stay code in the cell.
    expect(html).toContain('CLAUDE.md')
    expect(html).toMatch(/<code[^>]*>CLAUDE\.md<\/code>/)
  })

  it('honours right alignment from the separator', () => {
    const html = renderToStaticMarkup(<Markdown text={table} />)
    expect(html).toContain('text-align:right')
  })

  /*
   * The false positive worth guarding: a sentence with a pipe in it is not a table, and
   * treating it as one would swallow the paragraph after it.
   */
  it('leaves prose containing a pipe alone', () => {
    const html = renderToStaticMarkup(
      <Markdown text={'Run `a | b` to pipe it.\nThen read the output.'} />,
    )
