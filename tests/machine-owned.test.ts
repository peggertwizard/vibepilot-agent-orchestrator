...
import { blockingChanges, isDirty, isMachineOwned, workingCopyState } from '../src/main/git/repo'
...
describe('the merge that used to be impossible', () => {
  ...
  /*
   * "Genuinely" means the two overlap.
   *
   * This used to dirty a file the branch never touched, which is now — correctly — allowed
   * through: git leaves such a change exactly where it is, and refusing on it blocked every
   * merge in any project carrying a generated file. The refusal that still matters is the one
   * where the incoming work would land on top of what you have not saved.
   */
  it('still refuses when the user has unsaved work the branch would land on', async () => {
    gitq(['checkout', '-q', '-b', 'vp/2-thing'])
    writeFileSync(join(repo, 'app.js'), 'the agent rewrote this\n')
    gitq(['add', '-A'])
    gitq(['commit', '-qm', 'work'])
    gitq(['checkout', '-q', 'main'])
    ...
    if (!res.ok) {
      ...
      expect(res.reason, 'names the file').toContain('app.js')
    }
  })
})

/**
 * A file git calls modified but whose content is identical.
 *
 * `git status --porcelain` decides "modified" from mtime and size before it compares anything,
 * so a file a container rewrote byte-for-byte — or one whose line endings were normalised on
 * checkout — is reported as changed while `git diff` finds nothing in it. A generated
 * `importMap.js` in exactly that state blocked a merge, the user was asked whether to stash or
 * commit it, and the real diff was empty. There was never anything there.
 */
describe('what git says versus what actually changed', () => {
  it('does not block on a file rewritten with identical content', async () => {
    const before = await blockingChanges(repo)
    expect(before).toEqual([])

    // Same bytes, new mtime — precisely what a dev container regenerating a file does.
    writeFileSync(join(repo, 'app.js'), 'export const a = 1\n')
    execFileSync('node', ['-e', 'process.exit(0)'], { windowsHide: true })

    expect(await blockingChanges(repo), 'identical content is not a change').toEqual([])
  })

  it('still reports a file whose content really did change', async () => {
    writeFileSync(join(repo, 'app.js'), 'export const a = 2\n')
    expect(await blockingChanges(repo)).toContain('app.js')
  })

  it('never reports the app’s own folder', async () => {
    writeMemory()
    expect(await blockingChanges(repo)).toEqual([])
  })
})
