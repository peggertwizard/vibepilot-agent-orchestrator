...
describe('an API failure explains itself', () => {
  /**
   * The CLI puts the reason in `errors` when `result` is empty — which is exactly when
   * something went wrong early. This branch never read it, so what reached the board, the
   * Pilot and the user was the literal "The model call failed." with no information in it,
   * over and over: nobody could tell a rate limit from a dead API key, and neither could the
   * retry logic.
   */
  it('uses the errors array when result is empty', () => {
    const events = run([
      {
        type: 'result',
        ...
        is_error: true,
        result: '',
        errors: ['Rate limit reached for requests: retry after 2026-07-30T16:00:00Z'],
      },
    ])
    ...
    if (err?.type === 'agent:error') {
      ...
      expect(err.message).not.toBe('The model call failed.')
    }
  })

  it('still says something when the CLI gave nothing at all', () => {
    const events = run([{ type: 'result', subtype: 'error_during_execution', is_error: true }])
    const err = events.find((e) => e.type === 'agent:error')
    if (err?.type === 'agent:error') expect(err.message).toMatch(/gave no detail/i)
  })
})
