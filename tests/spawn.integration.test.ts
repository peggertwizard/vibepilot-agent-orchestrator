...
d('spawning a teammate', () => {
  ...
  beforeAll(async () => {
    ...
    /*
     * Auto-merge off, deliberately.
     *
     * What this file tests is **isolation**: the teammate's work lands on its own branch and
     * touches nothing else until somebody merges it. That guarantee is unchanged and is worth
     * asserting on its own, without a second feature deciding to merge halfway through.
     * `tests/auto-merge.test.ts` covers the landing behaviour.
     */
    updateProject(projectId, { autoMerge: 'off' })
    /*
     * And auto-start off. Two tests below assert the *manual* contract — that a proposed route
     * starts nothing until the user says so, and that accepting it starts the work. Both are
     * still guarantees under `autoStart: 'never'`, and both are meaningless if something else
     * pressed the button first. `tests/auto-start.test.ts` covers the automatic path.
     */
